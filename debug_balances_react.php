 ...
 *
 * Open (either works via company-slug rewrite):
 *   https://ultitech.io/debug_balances_react.php
 *   https://ultitech.io/ultimate/debug_balances_react.php
 *
 * Optional:
 *   ?probe=1  — try including modules/balances/index.php in a buffer (may redirect if not logged in)
 */
...
$roots = array_values(array_unique(array_filter([
    ...
    $scriptDir . DIRECTORY_SEPARATOR . 'ultimate',
    dirname($scriptDir) . DIRECTORY_SEPARATOR . 'ultimate',
])));
...
<p class="muted">Generated <?= db_h(date('c')) ?> · Delete <code>debug_balances_react.php</code> when done.</p>
...
db_row('DOCUMENT_ROOT/ultimate is dir?', $ultimateIsDir ? "YES — {$ultimateDir}" : 'no', $ultimateIsDir ? 'warn' : 'ok');
?>
</table>
...
if (!$primaryInfo['exists']) {
    ...
    if (!$distOk || !$ldOk || !$headOk) {
        $verdict = 'PARTIAL: React index.php found, but missing pieces — '
            . (!$ldOk ? 'ld-lib.php ' : '')
...
<p class="muted">React index found at:<br><?php foreach ($reactHits as $h) { echo '• ' . db_h($h) . '<br>'; } ?></p>
<?php endif; ?>
<?php if (!empty($legacyHits)): ?>
<p class="muted" style="color:#991b1b;">Non-React index found at:<br><?php foreach ($legacyHits as $h) { echo '• ' . db_h($h) . '<br>'; } ?></p>
<?php endif; ?>
...
echo '<table>';
if (!is_file($ldLib)) {
    db_row('ld-lib.php', 'MISSING — cannot probe URLs', 'bad');
} else {
    try {
...
if (!is_file($probe)) {
    echo '<div class="verdict bad">Cannot probe — index.php missing.</div>';
} else {
    ob_start();
    $err = null;
    try {
