...
if ($assets === null) {
    ...
    echo '<!DOCTYPE html><html><head><title>Order Details</title></head><body style="font-family:sans-serif;padding:2rem;">';
...
$initPayload = deliveries_load_order_details_payload($pdo, $_GET);
if (!($initPayload['ok'] ?? false)) {
    http_response_code(404);
    header('Content-Type: text/html; charset=utf-8');
    echo '<!DOCTYPE html><html><head><title>Order Not Found</title></head><body style="font-family:sans-serif;padding:2rem;">';
    echo '<h1>Order not found</h1>';
    echo '<p>' . htmlspecialchars((string) ($initPayload['error'] ?? 'Unknown error'), ENT_QUOTES, 'UTF-8') . '</p>';
    echo '</body></html>';
    exit;
}

$initData = $initPayload['data'] ?? [];
$orderRef = (string) ($initData['order']['invoice_ref'] ?? '');
...
$dlvConfig = [
    ...
    'orderId' => (int) ($initData['order']['id'] ?? 0),
    'data' => $initData,
];

$page_title = $orderRef !== '' ? ('Order ' . $orderRef) : 'Order Details';
$employeeHeaderTitle = '';
...
<head>
    ...
    <style>
        ...
        .main-content.order-details-react-root {
            width: 100% !important;
        ...
        .main-content.order-details-react-root #root { width: 100%; max-width: none; margin: 0; }
        @media (max-width: 1024px) { .main-content.order-details-react-root { padding: 1rem 0.875rem 1.5rem !important; } }
        @media (max-width: 767.98px) { .main-content.order-details-react-root { padding: 0.875rem 0.75rem 1.5rem !important; } }
...
$rootPath = '/';
$logoBase = '/';
$modulesLink = $initData['urls']['myDeliveries'] ?? '/select-module.php';
require_once dirname($deliveriesRoot) . '/includes/header_employee.php';
?>

<main class="main-content order-details-react-root">
    <noscript>
        <div class="alert alert-warning m-3">JavaScript is required to view order details.</div>
    </noscript>
    <div id="root"></div>
</main>
