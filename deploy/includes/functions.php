...
function ensurePettyCashSchema()
{
    ...
    try {
        ...
        if (function_exists('error_log')) {
            error_log('ensurePettyCashSchema error: ' . $ex->getMessage());
        }
    }
    $ensured = true;
}
...
function createPettyCashVoucher($data)
{
    ...
    $stmt = $pdo->prepare("INSERT INTO petty_cash_vouchers 
        (voucher_number, date, custodian_id, category, description, amount, receipt_path, created_by, is_restricted) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
