...
// Date Filters
$startDate = $_GET['start_date'] ?? date('Y-m-01'); // Default to 1st of this month
$endDate = $_GET['end_date'] ?? date('Y-m-t');     // Default to end of this month

// Revenues
$sqlRev = "SELECT a.name, COALESCE(SUM(ji.credit), 0) - COALESCE(SUM(ji.debit), 0) as amount 
           FROM erp_accounts a 
...
// Expenses
$sqlExp = "SELECT a.name, COALESCE(SUM(ji.debit), 0) - COALESCE(SUM(ji.credit), 0) as amount 
           FROM erp_accounts a 
           LEFT JOIN erp_journal_items ji ON a.id = ji.account_id 
           LEFT JOIN erp_journal_entries je ON ji.journal_id = je.id 
