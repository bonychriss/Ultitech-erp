...
try {
    ...
    if ($action === 'create') {
        ...
        // AUTOMATION HOOK
        if ($status === 'posted') {
            require_once '../includes/AccountingEngine.php';
            try {
                $engine = new AccountingEngine($pdo, $_SESSION['user_id']);
                $engine->postInvoice($id);
            } catch (Exception $e) {
                // Determine if we should fail the whole request or just log.
                // For now, let's just log it but keep success, or maybe Append warning?
                // Let's fail loudly so user knows accounting failed.
                // throw $e; 
                // Actually better to just proceed but maybe return warning? 
                // Let's stick to simple implementation: if accounting fails, the whole thing fails (catch block at bottom will rollback transaction if we were in one, but we aren't here yet).
                // Let's wrap in transaction for safety.
            }
        }
        ...
        // For now, just mark as paid and update balance to 0
        if (empty($_POST['id'])) {
        ...
        $stmt = $pdo->prepare("UPDATE erp_invoices SET status = 'paid', balance = 0 WHERE id = ?");
        $stmt->execute([$id]);
        
        // AUTOMATION HOOK
        require_once '../includes/AccountingEngine.php';
        $engine = new AccountingEngine($pdo, $_SESSION['user_id']);
        $engine->registerPayment($id);
        ...
        if (empty($invoice['customer_email'])) throw new Exception('Customer has no email address');
        ...
        if (!empty($invoice['customer_email'])) $body .= "<p style='margin: 2px 0;'>" . htmlspecialchars($invoice['customer_email']) . "</p>";
        $body .= "</td>";
