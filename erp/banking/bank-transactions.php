...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .account-info { background: white; border-radius: 8px; border: 1px solid #e0e0e0; padding: 20px; margin-bottom: 24px; display: flex; justify-content: space-between; align-items: center; }
...
<?php include '../includes/sidebar.php'; ?>
    ...
    <div class="container">
        <div class="account-info">
            <div>
                <div style="font-size: 0.875rem; color: #5f6368; margin-bottom: 4px;"><?= htmlspecialchars($account['bank_name']) ?> â€¢ <?= htmlspecialchars($account['account_number']) ?></div>
                <div style="font-size: 1.5rem; font-weight: 600; color: #137333;">TSh <?= number_format($account['current_balance'], 2) ?></div>
            </div>
            ...
            <table class="table">
                ...
                <tbody>
                    <?php if (empty($transactions)): ?>
                        <tr><td colspan="7" style="text-align: center; padding: 40px; color: #5f6368;">No transactions yet. Import from bank statement.</td></tr>
                    <?php else: ?>
