...
// Schema Migration: Check for missing columns (Self-Healing)
try {
    $checkCol = function ($col, $def) use ($pdo) {
        $stmt = $pdo->prepare("SHOW COLUMNS FROM erp_quotes LIKE ?");
        $stmt->execute([$col]);
        if (!$stmt->fetch()) {
    ...
    $checkCol('created_at', "TIMESTAMP NOT NULL DEFAULT current_timestamp()");
    $checkCol('updated_at', "TIMESTAMP NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()");
    $checkCol('quote_date', "DATE DEFAULT NULL");
...
try {
    ...
    foreach ($quotes as &$q) {
        if (!isset($q['quote_date']) && $hasQuoteDate && isset($q[$orderCol])) {
            $q['quote_date'] = $q[$orderCol];
        }
        if ($expiryCol && !isset($q['expiry_date']) && isset($q[$expiryCol])) {
            $q['expiry_date'] = $q[$expiryCol];
        }
        if (!isset($q['total_amount']) && isset($q[$amountField])) {
            $q['total_amount'] = $q[$amountField];
        }
    }
    unset($q);
    ...
    <div class="page-wrapper">
        ...
        <div class="header">
            ...
            <div class="header-actions">
                ...
                <button id="bulkInvoiceBtn" class="btn btn-success" style="display:none;"
        ...
        <div class="container">
            <?php if ($error_message): ?>
                <div class="card"
                    style="padding: 16px; background: #fee2e2; color: #dc2626; border-color: #fecaca; margin-bottom: 20px;">
            ...
            <div class="card">
                ...
                <form method="GET" class="filters-toolbar">
                    <div class="search-box">
                        ...
                        <input type="text" name="search" placeholder="Search Number or Customer..."
                    ...
                    <tbody>
                        <?php if (empty($quotes)): ?>
                            <tr>
                                <td colspan="8" style="text-align: center; padding: 48px; color: #6b7280;">
                                    <div style="font-size: 3rem; margin-bottom: 16px; color: #d1d5db;"><i
                        ...
                        <?php else: ?>
                            <?php foreach ($quotes as $q):
                                $canConvert = ($q['status'] === 'accepted' || $q['status'] === 'sent'); ?>
                                <tr>
                                    <td>
                                        <input type="checkbox" class="quote-checkbox" value="<?= $q['id'] ?>"
                                            data-status="<?= $q['status'] ?>" onchange="updateBulkButton()" <?= $canConvert ? '' : 'disabled' ?>>
                                    </td>
                                    <td style="font-weight: 500; font-family: monospace; color: #111827;">
                                        <?= htmlspecialchars($q['quote_number']) ?></td>
                                    <td><?= htmlspecialchars($q['customer_name']) ?></td>
                                    <td><?= date('M d, Y', strtotime($q['quote_date'] ?? $q['created_at'] ?? date('Y-m-d'))) ?></td>
                                    <td><?= isset($q['expiry_date']) && $q['expiry_date'] ? date('M d, Y', strtotime($q['expiry_date'])) : '-' ?>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <span
                                            class="badge <?= $statusClass[$q['status']] ?? 'badge-info' ?>"><?= ucfirst($q['status']) ?></span>
                                    </td>
