...
<body>
    ...
    <main class="page-wrapper">
        ...
        <div class="action-bar">
            <div class="btn-group">
                ...
                <?php elseif ($invoice['status'] === 'unpaid' || $invoice['status'] === 'posted' || $invoice['status'] === 'partial'): ?>
                    <button class="btn btn-primary" onclick="openPaymentModal()">Register Payment</button>
                <?php endif; ?>
            ...
            <div class="pipeline-widget">
                <?php
                $stages = ['draft' => 'Draft', 'posted' => 'Posted', 'in_payment' => 'In Payment', 'paid' => 'Paid'];

                // Map current status to pipeline stage
                $currentStatusForPipeline = $invoice['status'];
                // Unpaid/Partial maps to Posted visually in Odoo (usually), but let's show In Payment if it's there.
                if ($invoice['status'] === 'unpaid')
                    $currentStatusForPipeline = 'posted';
                if ($invoice['status'] === 'partial')
                    $currentStatusForPipeline = 'posted'; // Or in_payment? Let's use posted.
                
                foreach ($stages as $key => $label):
                    $activeClass = ($currentStatusForPipeline === $key) ? 'active' : '';
                    ?>
        ...
        <div class="sheet-container">
            <div class="sheet">
                ...
                <?php elseif ($invoice['status'] === 'in_payment'): ?>
                    <div class="ribbon"><span style="background-color: #17a2b8;">IN PAYMENT</span></div>
                <?php elseif ($invoice['status'] === 'partial'): ?>
                    <div class="ribbon"><span style="background-color: #ffc107; color: #333;">PARTIAL</span></div>
                <?php endif; ?>

                <div class="sheet-header-title"
                    style="display: flex; justify-content: space-between; align-items: flex-start;">
                    <h1 class="sheet-title" style="margin: 0; color: #000; font-size: 2.5rem;">Invoice #
                        <?= htmlspecialchars($invoice['invoice_number']) ?>
                    </h1>
                    <div style="text-align: right;">
                        <img src="../../assets/images/Untitled.jpg" alt="Logo"
                            style="height: 60px; margin-bottom: 10px;">
                        <div style="font-weight: bold; font-size: 1rem; color: #111; text-transform: uppercase;">
                            Ultimate General Trading Company</div>
                    ...
                    <div class="left-col" style="width: 100%;">
                        <div class="form-group">
                            ...
                            <div class="form-value" style="font-size: 1rem; line-height: 1.6;">
                                <strong
                                    style="font-size: 1.1rem; color: #111;"><?= htmlspecialchars($invoice['customer_name']) ?></strong><br>
                                <?php if (!empty($invoice['customer_address'])): ?>
                                    <span
                                        style="color: #555;"><?= nl2br(htmlspecialchars($invoice['customer_address'])) ?></span><br>
                                <?php endif; ?>
                                <?php if (!empty($invoice['customer_email'])): ?>
                                    <span
                                        style="color: #666;"><?= htmlspecialchars($invoice['customer_email']) ?></span><br>
                                <?php endif; ?>
                                <?php if (!empty($invoice['customer_tax_id'])): ?>
                                    <span style="color: #666;">Tax ID:
                                        <?= htmlspecialchars($invoice['customer_tax_id']) ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                ...
                <div
                    style="margin-top: 10px; margin-bottom: 30px; border: 1px solid #000; display: flex; text-align: left;">
                    ...
                    <div style="display: flex; justify-content: space-between; margin-top: 20px;">
                        ...
                        <div style="flex: 1; padding-right: 20px;">
                            <?php if (!empty($bankAccounts)): ?>
                                <h4
                                    style="font-size: 0.95rem; font-weight: 600; color: #111; margin-bottom: 8px; text-decoration: underline;">
                                    Bank Details</h4>
                                <div style="font-size: 0.85rem; color: #333; line-height: 1.5;">
                                    <?php foreach ($bankAccounts as $acc): ?>
                                        <div style="margin-bottom: 8px;">
                                            ...
                                            Branch: <?= htmlspecialchars($acc['branch']) ?> | Currency:
    ...
    <div id="paymentModal" class="modal"
        ...
        <div class="modal-content"
            style="background-color: #fefefe; margin: 15% auto; padding: 24px; border: 1px solid #888; width: 500px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h2 style="font-size: 1.25rem; color: #2c3e50;">Register Payment</h2>
            ...
            <form id="paymentForm" onsubmit="submitPayment(event)">
                ...
                <div class="form-group" style="margin-bottom: 15px; display: block;">
                    ...
                    <select name="bank_account_id" required
                        ...
                        <?php foreach ($bankAccounts as $acc): ?>
                            <option value="<?= $acc['id'] ?>">
                                <?= htmlspecialchars($acc['bank_name'] . ' - ' . $acc['currency']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group"
                    ...
                    <div>
                        <label style="display: block; margin-bottom: 5px; font-weight: 500;">Payment Amount</label>
                        <input type="number" name="amount" step="0.01" value="<?= $invoice['balance'] ?>" required
                            style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                    </div>
                    <div>
                ...
                <div class="form-group" style="margin-bottom: 20px; display: block;">
                    <label style="display: block; margin-bottom: 5px; font-weight: 500;">Memo / Reference</label>
                    <input type="text" name="reference" value="<?= $invoice['invoice_number'] ?>"
                        style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                </div>
    ...
    <script>
        ...
        async function submitPayment(e) {
            e.preventDefault();
            if (!confirm('Confirm register payment?')) return;
