...
global $pdo;
$id = $_GET['id'] ?? 0;
$stmt = $pdo->prepare("SELECT q.*, c.name as customer_name, c.email as customer_email, c.phone as customer_phone, c.address as customer_address, c.tax_id as customer_tax_id FROM erp_quotes q JOIN erp_customers c ON q.customer_id = c.id WHERE q.id = ?");
$stmt->execute([$id]);
$quote = $stmt->fetch();
if (!$quote)
    die("Quote not found");
...
<body>
        ...
        <div class="action-bar">
            <div class="btn-group">
                ...
                <?php if ($quote['status'] === 'draft'): ?>
                    ...
                    <button onclick="confirmQuote(<?= $id ?>)" class="btn btn-primary">Confirm</button>
                <?php elseif ($quote['status'] === 'sent'): ?>
                    <button onclick="confirmQuote(<?= $id ?>)" class="btn btn-primary">Confirm Sale</button>
                    <button class="btn btn-secondary">Resend Email</button>
                <?php elseif ($quote['status'] === 'accepted'): ?>
                    <button onclick="confirmQuote(<?= $id ?>)" class="btn btn-primary">Create Sales Order</button>
        ...
        <div class="sheet-container">
            <div class="sheet">
                <div class="sheet-header-title"
                    ...
                    <h1 class="sheet-title" style="margin: 0; color: #000; font-size: 2.5rem;">Quotation #
                    ...
                    <div class="left-col" style="width: 100%;">
                        ...
                        <div class="form-group">
                            ...
                            <div class="form-value" style="font-size: 1rem; line-height: 1.6;">
                                <strong
                                    style="font-size: 1.1rem; color: #111;"><?= htmlspecialchars($quote['customer_name']) ?></strong><br>
                                <?php if (!empty($quote['customer_address'])): ?>
                                    <span
                                        style="color: #555;"><?= nl2br(htmlspecialchars($quote['customer_address'])) ?></span><br>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($quote['customer_tax_id'])): ?>
                                    <span style="color: #666;">Tax ID:
                                        <?= htmlspecialchars($quote['customer_tax_id']) ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                ...
                <div
                    ...
                    <div style="flex: 1; padding: 2px;">
                        ...
                        <div style="font-size: 0.85rem;">
                            <?= $quote['expiry_date'] ? date('d/m/Y', strtotime($quote['expiry_date'])) : '-' ?>
                        </div>
                    </div>
                    <div style="flex: 1; padding: 2px;">
    ...
    <script>
        async function updateStatus(id, status) {
            confirmAction('Change status?', 'Change status to ' + status + '?', 'Yes, update', async () => {
                try {
        ...
        async function confirmQuote(id) {
            confirmAction('Confirm Sale?', 'This will create a Sales Order and Delivery Order. Continue?', 'Yes, Confirm', async () => {
                try {
                    const formData = new FormData();
                    formData.append('action', 'convert_quote');
                    ...
                    if (result.success) {
                        showToast('success', 'Sales Order Created!');
                        setTimeout(() => window.location.href = 'view-sales-order.php?id=' + result.id, 1000);
                    } else {
                        showToast('error', 'Failed: ' + result.message);
                    }
                } catch (error) {
        ...
        async function sendEmailQuote(id) {
            confirmAction('Send Email?', 'Send this quotation by email?', 'Yes, send', async () => {
                // Show loading state
                const btn = document.querySelector('.btn-action');
                const originalText = btn.innerText; // Use innerText to match existing element
                if (btn) {
