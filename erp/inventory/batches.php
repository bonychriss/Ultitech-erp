...
try {
    ...
    if (defined('APP_ENV') && APP_ENV === 'production') {
        ...
        $error_message = "Unable to load batches. Please ensure the database is updated.";
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 2000; justify-content: center; align-items: flex-start; padding-top: 50px; }
        .modal-content { background: white; width: 95%; max-width: 600px; border-radius: 8px; padding: 24px; box-shadow: 0 8px 24px rgba(0,0,0,0.2); }
...
<div class="page-wrapper">
    <div style="padding: 16px 24px 0; text-align: right;"><div class="header-actions">
            ...
            <button id="openAddBatchBtn" onclick="openAddBatchModal()" class="btn btn-primary">+ Add Batch</button>
        </div></div>
    ...
    <script>
        function openAddBatchModal() {
            var m = document.getElementById('addBatchModal');
            if (!m) return;
        ...
        // Fallback listener on the button to ensure it opens even if inline handler blocked
        var openBtn = document.getElementById('openAddBatchBtn');
        if (openBtn) {
            openBtn.addEventListener('click', function(ev){ ev.preventDefault(); openAddBatchModal(); });
        }
        ...
        window.addEventListener('keydown', function(ev){ if (ev.key === 'Escape') closeAddBatchModal(); });
    </script>
</div>
</body>
</html>
