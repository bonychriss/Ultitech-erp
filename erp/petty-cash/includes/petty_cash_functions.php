    ...
    function getPettyCashVoucher($id)
    {
        ...
        $stmt = $pdo->prepare(
            "SELECT v.*,
                ...
                exp_acc.name AS expense_account_name
             FROM petty_cash_vouchers v
             LEFT JOIN users c ON v.custodian_id = c.id
             LEFT JOIN users a ON v.approved_by = a.id
             LEFT JOIN users cr ON v.created_by = cr.id
             LEFT JOIN financial_accounts pc_acc ON pc_acc.id = v.petty_cash_account_id
             LEFT JOIN financial_accounts exp_acc ON exp_acc.id = v.expense_account_id
             WHERE v.id = ?"
        );
        $stmt->execute([(int) $id]);
        ...
        try {
            $stmt = $pdo->prepare(
                ...
                (voucher_number, date, custodian_id, category, description, amount, receipt_path, created_by, status,
                 petty_cash_account_id, expense_account_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)"
            );
            $stmt->execute(array_merge($paramsBase, [
                !empty($data['petty_cash_account_id']) ? (int) $data['petty_cash_account_id'] : null,
                !empty($data['expense_account_id']) ? (int) $data['expense_account_id'] : null,
            ]));

            return (int) $pdo->lastInsertId();
        } catch (PDOException $e) {
            error_log('createPettyCashVoucher with accounts: ' . $e->getMessage());
        }
...
if (!function_exists('approvePettyCashVoucher')) {
    function approvePettyCashVoucher($voucher_id, $approved_by)
    {
        ...
        try {
            ...
            $voucher['approved_by'] = (int) $approved_by;

            $balancesIntegration = dirname(__DIR__, 3) . '/modules/petty-cash/includes/balances_integration.php';
            if (is_file($balancesIntegration)) {
                require_once $balancesIntegration;
                if (function_exists('petty_cash_post_voucher_to_balances')) {
                    $postResult = petty_cash_post_voucher_to_balances($pdo, $voucher);
                    if (empty($postResult['success'])) {
                        throw new RuntimeException((string) ($postResult['message'] ?? 'Could not post voucher to Balances.'));
                    }
                }
            }
...
if (!function_exists('pettyCashGetFinancialAccount')) {
    function pettyCashGetFinancialAccount(int $id, bool $forUpdate = false): ?array
    {
        ...
        $stmt = $pdo->prepare(
            "SELECT id, name, type, currency, current_balance, opening_balance, status, parent_id
