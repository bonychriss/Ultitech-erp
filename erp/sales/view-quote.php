...
<body>
    ...
    <script>
        ...
        async function convertToInvoice(id) {
            confirmAction('Confirm Sale?', 'Confirm Sale and Create Invoice?', 'Yes, Confirm', async () => {
                try {
                    const formData = new FormData();
                    ...
                    const result = await response.json();
                    if (result.success) {
                        showToast('success', 'Order Confirmed! Invoice Created.');
                        setTimeout(() => window.location.href = 'view-invoice.php?id=' + result.invoice_id, 1000);
                    } else {
                        showToast('error', 'Failed: ' + result.message);
                    }
                } catch (error) {
