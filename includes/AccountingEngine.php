...
class AccountingEngine {
    ...
    public function createJournalEntry($date, $description, $referenceType, $referenceId, $items) {
        ...
        // Insert Items (schema-compatible account column)
        $journalAccountCol = resolveExistingColumn('erp_journal_items', 'account_id', ['gl_account_id', 'account']);
        if (!$journalAccountCol) {
            throw new Exception("Journal items account column not found.");
        }
        $itemStmt = $this->pdo->prepare("INSERT INTO erp_journal_items (journal_id, {$journalAccountCol}, debit, credit) VALUES (?, ?, ?, ?)");
