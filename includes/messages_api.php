...
try {
    ...
    if ($action === 'send') {
        ...
        if ($recipientId) {
            // Private Message
            $stmt = $pdo->prepare('INSERT INTO messages (sender_id, recipient_id, message, reply_to_id, is_read) VALUES (?, ?, ?, ?, 0)');
            $stmt->execute([$uid, $recipientId, $message, $replyToId]);
            $msgId = $pdo->lastInsertId();
        } else {
            // Group Message (Default to global if no group specified)
            $gid = $gid ?: $groupId;
            $stmt = $pdo->prepare('INSERT INTO messages (sender_id, group_id, message, reply_to_id, is_read) VALUES (?, ?, ?, ?, 0)');
            $stmt->execute([$uid, $gid, $message, $replyToId]);
            $msgId = $pdo->lastInsertId();
        }
            ...
            for ($i = 0; $i < $count; $i++) {
                if ($_FILES['files']['error'][$i] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($tmpName, $targetPath)) {
                        $relPath = 'assets/uploads/chat/' . $newName;
                        $stmt = $pdo->prepare('INSERT INTO message_attachments (message_id, file_path, file_name, mime_type, size_bytes) VALUES (?, ?, ?, ?, ?)');
                        $stmt->execute([$msgId, $relPath, $name, $mime, $size]);
                    }
                }
            }
        }
    ...
    $chatType = $_GET['chat_type'] ?? 'group'; // 'group' or 'private'
    $chatId = isset($_GET['chat_id']) ? (int)$_GET['chat_id'] : 0;

    if ($chatId <= 0) {
        // Fallback to global group if no chat selected
        $chatType = 'group';
        $chatId = $groupId; 
    }
    ...
    if ($chatType === 'private') {
        // Private chat: messages between current user ($uid) and other user ($chatId)
        $sql = "SELECT m.id, m.sender_id, m.message, m.created_at, m.reply_to_id, m.edited_at, m.is_deleted, u.full_name, u.profile_photo,
        ...
        $stmt->execute([$uid, $chatId, $chatId, $uid, $since, $uid]);
    } else {
        // Group chat
        $sql = "SELECT m.id, m.sender_id, m.message, m.created_at, m.reply_to_id, m.edited_at, m.is_deleted, u.full_name, u.profile_photo,
                       r.message AS reply_message, ru.full_name AS reply_full_name
    ...
    foreach ($rows as $r) {
        ...
        $out[] = [
            ...
            'profile_photo' => isset($r['profile_photo']) ? (string)$r['profile_photo'] : null,
