// Voucher form functionality (Version 9 cache-bust)
console.log('[voucher-v5.js] Loaded version 10 (primary)');
let itemCount = 0;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    ensureSingleHeader();
    // Try restore from local draft first; otherwise start with one blank item
    ...
    calculateTotal();
    // Autosave: on any input/change and when leaving the page
...
function addVoucherItem(data = null) {
    ...
    // Pick a sensible default for the first item so server receives a non-empty type
    ...
    // Remove any server-side fallback row if present
    try { document.querySelectorAll('.server-fallback').forEach(el => el.remove()); } catch(_){}
    // If this is the first item and we have a default type, propagate lock so all future items match
...
function lockPaymentType(selectedType) {
    ...
    paymentSelects.forEach((select, idx) => {
        ...
        if (!isFirst) { 
            ...
            select.style.cursor = ''; 
            // Remove hidden input for first item since it's not disabled
            const hiddenInput = select.parentElement.querySelector('input[type="hidden"][name="payment_type[]"]');
...
function removeVoucherItem(itemId) {
    ...
    if (item) { item.remove(); calculateTotal(); }
}
...
function saveDraft(isAuto = false) {
    ...
    try { localStorage.setItem(getDraftKey(), JSON.stringify(payload)); } catch(_) { /* ignore quota */ }

    if (!isAuto) {
        // subtle toast only on manual trigger (not used for autosave)
...
function loadDraft() {
    try {
        ...
        Object.keys(data).forEach(key => {
            if (key.endsWith('[]')) return; // skip array fields here
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (voucherForm) {
        voucherForm.addEventListener('submit', function(event) {
            // Re-enable all disabled payment type selects AND ensure hidden inputs are set
            const paymentSelects = document.querySelectorAll('select[name="payment_type[]"]');
            paymentSelects.forEach(select => { 
                select.disabled = false;
                // Also ensure hidden inputs match the select values
                const hiddenInput = select.parentElement.querySelector('input[type="hidden"][name="payment_type[]"]');
                if (hiddenInput) {
