...
- **Financial overview** � sales, expenses, profit, receivables, inventory alerts
- **Sales & revenue** � invoice totals, collections, customer rankings, invoice detail
- **Finance & expenses** � income vs expense, voucher tracking, expense breakdown
- **Weekly mission performance** � team completion rates, leaderboard, mission status
- **Business Reports hub card** � links to the multi-domain report editor (`modules/sales-reports/`)
- **Advanced Sales Analytics** (`smart_report_sales.php`) � deep sales drill-down with monthly matrices, targets, pipeline, AR aging, and a data-verification gate
- **Rep detail** (`smart_report_rep_detail.php`) � per-sales-employee quotations/invoices with rule-based or AI insights
...
| Module-specific role | **Not implemented** � any logged-in user who can open the module picker sees this module |
| Admin-only | **No** � listed for all users in `select-module.php` |
...
**Note:** `.da-nav` / `.da-shell` classes are output in PHP but **no CSS rules for `.da-nav` exist** in `assets/css/analytics.css` � tabs may appear minimally styled.
...
**Loading states:** None � full page SSR.
...
**Not linked from main tab navigation** � reachable only via direct URL  
...
No dedicated KPI cards � data is in tables/charts. Top Performer highlight shows name, department, points, completion %.
...
All main-dashboard charts use **Chart.js** (loaded from CDN in `layout.php`). Data is embedded at page render time � **no chart API**.
...
| Employee Performance | Overview, Performance | Bar (horizontal if >6 labels) | Employee names | Completion % (0�100) | `analytics_employee_performance_chart()` ? `wm_leaderboard()` |
...
| Top Customers by Revenue | `smart_report_ranking_matrices()` | Customer � month |
| Dormant Products | `smart_report_ranking_matrices()` | Product � month (90+ days since last sale) |
| Open Deals by Month | `smart_report_pipeline_matrices()` | Order � month |
...
- **Monthly Top Performer** � employee with highest `SUM(award_points)` and best `AVG(completion_rate)` since first day of current month (`analytics_monthly_top_performer()`).

### Sales Analytics � Data Checker

| Aspect | Detail |
| ------ | ------ |
| Trigger | Every load of `smart_report_sales.php` |
| What it checks | Displayed revenue, invoice count, gross profit math, margin %, team sales, rep sum, pipeline count/value, matrix totals vs re-queried �ground truth� |
| Pass condition | All checks within tolerance (`smart_report_sales_amounts_close()`, default �0.01) |
...
| Status | `status` | `""` | Mission chart � zeroes non-selected statuses |
| ERP module context | `module=analytics` | Required hidden field | Sidebar/navigation context only |

**Submission:** HTML GET form � **Apply** submits; **Reset** clears to `?module=analytics`.
...
- `tree_type` � matrix grouping (default `customer_group`)
- `value_qty` � `value` or `quantity`
...
Yearly target � (`count(months in range)` / 12) via `smart_report_sales_rep_period_target()`.

### Performance notes

- Invoice detail limited to **200 rows** (`analytics_sales_rows()`)
- Finance rows capped at **200** after merge
- Top customers limited to **10�15** depending on page
...
**Main dashboards:** No dedicated `.js` files � charts only.
...
| Company SQL scoping | ?? Partial � most invoice/sales queries scoped; several gaps (see [Known Issues](#20-known-issues--limitations)) |
...
| PDF | ? | Not in analytics module | � |
| Excel | ? | � | � |
| Print | ? | No dedicated print view | � |

CSV includes UTF-8 BOM (`EF BB BF`). Filename pattern: `analytics-{section}-{date}.csv`.

**Sales Reports module** (linked from nav) has its own PDF/export pipeline � outside this folder.
...
| Rep AI fetch (React) | `.catch(() => {})` � silent failure, keeps initial rules insights |
...
| `smart_report_sales.php` | **Very heavy** � full drilldown + 3 matrix builders + verification re-queries ground truth |
...
| Low stock query | **No company scope** � full table scan |
...
| Tab navigation | ? | `layout.php` ? `analytics_nav()` | � | `.da-nav` CSS missing |
...
| Order Fulfillment section | ? | Sales Analytics | � | "Coming soon" panel |
| PDF/Excel export | ? | � | � | |
| Role-based access control | ? | � | � | Login only |
| Real-time / AJAX dashboard refresh | ? | Main pages | � | Full page reload on filter |
| smart_report hub page | ? | � | � | Removed (per codebase; no `smart_report.php`) |

---

## 20. Known Issues & Limitations

1. **`api/export.php` require path** � Uses `../../includes/functions.php` instead of `../../../includes/functions.php`; likely broken.
2. **Sales Analytics not in navigation** � `smart_report_sales.php` has no tab link; users must know the URL.
3. **Data checker blocks entire Sales Analytics** � Any verification mismatch hides all content (strict gate).
4. **Order Fulfillment** � UI shows "Coming soon" despite partial fulfillment metrics computed in `smart_report_sales_drilldown()`.
5. **`.da-nav` unstyled** � No CSS rules for tab navigation class.
6. **Company scope gaps:**
   - `analytics_low_stock_count()` � no company filter
   - `finance.php` expense category + pending voucher queries � no `analytics_append_company_scope()`
   - `erp_expenses` / `expenses_requests` fallbacks � no company scope
   - Some `performance_points` queries on overview � no company filter
7. **Pending Payments KPI** � Uses all-time `SUM(balance_due)`, not limited to selected date range (subtitle says "Outstanding receivables").
8. **Mission Completion KPI subtitle** � Says "Current week team average" but uses selected week from filters when `week_start` set on overview (week filter only shown on performance page by default).
9. **`debug_pdo_tables.php`** � Hard-coded `company_slug = 'roadmaster'`; dev artifact, not production UI.
10. **Rep detail JSX via Babel in browser** � Not pre-built; depends on CDN availability.
11. **`analytics_url_company_mismatch()`** � Defined but unused in analytics bootstrap.
12. **Hub card CSS for smart report** � `.da-hub-card--smart` styles exist but smart report hub card was removed from index.
...
6. User changes **From/To** dates and clicks **Apply** � full page reload with new query string.
7. User clicks **Export CSV** � browser requests `api/export.php?section=overview&...` (if path works).
...
| Authorization | Minimal � login + rep company check only |
...
| Related module | `modules/sales-reports/` � multi-domain AI business report writer (React + TinyMCE) |
...
| ---------- | ----- | ------------------ |
| `sales` | Sales Report | `invoices`, `sales_orders`, `sales_targets`, � (unchanged) |
...
Section catalogs per domain in `reportEngineSectionCatalog()` � e.g. Executive Summary, KPI Overview, domain-specific sections (Spend Analysis, Revenue Analysis, Trip Performance, Stock Movement), plus Recommendations, Action Plan, Conclusion.
...
- `CreateReportTypeModal.jsx` � Step 1: domain picker; Step 2: date range + filters; Sales keeps monthly/quarterly presets.
- `ReportCard.jsx` � domain badge on list rows.
...
1. Fleet reporting limited to delivery trip data � no fuel/maintenance/vehicle registry tables in ERP.
...
`modules/sales-reports/tools/test-domains-cli.php` � verifies snapshot + ERP block generation for all non-sales domains.
