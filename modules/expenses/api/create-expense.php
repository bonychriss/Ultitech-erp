...
} elseif (!$isDraft && !empty($_POST['require_receipt'])) {
...
try {
    ...
    if ($isDraft) {
        $stmt = $pdo->prepare("INSERT INTO erp_expenses
            (expense_number, date, payee, account_id, source_account_id, amount, tax_amount, currency_code, payment_method, description, status, is_posted, created_by, attachment)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', 0, ?, ?)");
        $stmt->execute([
            $expense_number,
            $date,
        ...
        $stmt = $pdo->prepare("INSERT INTO erp_expenses
            (expense_number, date, payee, account_id, source_account_id, amount, tax_amount, currency_code, payment_method, description, status, is_posted, created_by, attachment, approved_by, approved_at)
    ...
    if (!$isDraft) {
        ...
        $message = 'Expense saved as draft. Post it later after completing the details.';
    }
