<?php
/**
 * Three-dot actions menu for petty cash voucher table rows.
 *
 * @var array  $voucher           Row with id, status
 * @var string $viewUrl           View voucher URL
 * @var bool   $can_manage        Whether user can approve/reject/cancel
 * @var string $reject_form_id    Hidden reject form element id
 */
if (!isset($voucher) || !is_array($voucher)) {
    return;
}
...
$reject_form_id = (string) ($reject_form_id ?? ('reject-v-' . $vid));
...
$approveConfirmScript = (string) ($_SERVER['SCRIPT_NAME'] ?? '');
$approveConfirmBase = (strpos($approveConfirmScript, '/vouchers/') !== false)
    ? 'confirm-approve.php'
    : 'vouchers/confirm-approve.php';
$approve_confirm_url = $approveConfirmBase . '?' . http_build_query(array_filter([
...
if (empty($GLOBALS['_pc_row_menu_assets'])) {
    ...
    <style>
        ...
        .pc-row-menu-item.view { color: #7c3aed; }
...
<div class="pc-row-menu" data-pc-row-menu>
    <button type="button" class="pc-row-menu-btn" aria-label="Voucher actions" aria-haspopup="true" onclick="event.stopPropagation(); pcToggleRowMenu(this);">
        <i class="fas fa-ellipsis-vertical" aria-hidden="true"></i>
    </button>
    <div class="pc-row-menu-panel" hidden role="menu">
        <a href="<?= htmlspecialchars($viewUrl) ?>" class="pc-row-menu-item view" role="menuitem">
            <i class="fas fa-eye"></i> View
        </a>
        <?php if ($showApprove): ?>
            <div class="pc-row-menu-divider"></div>
        ...
        <?php endif; ?>
        <?php if ($showCancel): ?>
            <?php if ($showApprove): ?><?php else: ?><div class="pc-row-menu-divider"></div><?php endif; ?>
            <form method="POST" class="m-0" onsubmit="return confirmCancel(<?= $cancelApproved ? 'true' : 'false' ?>);">
                <input type="hidden" name="action" value="cancel_voucher">
