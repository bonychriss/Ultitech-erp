...
/** @var string $invoiceCreatePageTitle */
...
<head>
    ...
    <title><?= htmlspecialchars($invoiceCreatePageTitle ?? 'Create Invoice') ?></title>
    ...
    <style>
        * {
        ...
        .inv-currency-trigger:focus,
        .inv-currency-picker.is-open .inv-currency-trigger {
            border-color: #3b82f6;
        ...
        .inv-currency-option:hover,
        .inv-currency-option.is-selected { background: #eff6ff; }
        .inv-currency-option .code { font-weight: 700; color: #0f172a; min-width: 42px; }
        .inv-currency-option .name { color: #64748b; }
        .inv-currency-option.is-checked { background: #eff6ff; }
        .inv-currency-check {
            width: 16px;
            height: 16px;
        ...
        .inv-currency-option.is-checked .inv-currency-check {
            background: #2563eb;
            border-color: #2563eb;
        }
        ...
        .inv-rate-grid { display: grid; gap: 10px; margin-top: 4px; }
        .inv-rate-row { display: grid; grid-template-columns: 72px 1fr; gap: 10px; align-items: center; }
        .inv-rate-row label { font-size: 12px; font-weight: 700; color: #334155; }
        .inv-rate-hint { font-size: 12px; color: #64748b; margin-top: 6px; line-height: 1.45; }
        .inv-rate-hint.is-loading { color: #94a3b8; }
        .inv-summary-equiv { font-size: 11px; color: #64748b; font-weight: 600; margin-top: 2px; }
        .inv-type-badge {
            display: inline-flex;
            align-items: center;
            gap: 6px;
...
<body class="page-sales-invoice-create">
    ...
    <script>
        window.APP_DATA = {
            ...
            isUltimate: <?= json_encode(function_exists('isUltimate') && isUltimate()) ?>,
            supportsTruckInvoices: <?= json_encode(function_exists('salesSupportsTruckInvoices') && salesSupportsTruckInvoices()) ?>,
            ...
            defaultCurrency: <?= json_encode($defaultCurrency ?? 'TZS') ?>,
            currencyOptions: <?= json_encode($invoiceCurrencyOptions ?? ['TZS' => ['name' => 'Tanzanian Shilling', 'flag' => 'tz']]) ?>,
            initialExchangeRate: <?= json_encode(number_format((float) ($initialExchangeRate ?? 1), 4, '.', '')) ?>,
            initialExchangeRates: <?= json_encode($initialExchangeRates ?? ['TZS' => '1.0000']) ?>,
            exchangeRateApiUrl: <?= json_encode($exchangeRateApiUrl ?? '../payments/exchange_rate.php') ?>
        };
    </script>
    ...
    <script type="text/babel">
        ...
        function InvoiceCreateApp() {
            const { products, customers, currentUserName, error, catalogueUrl, customerCatalogueUrl, stockUploadsBase, isRoadmaster, isUltimate, supportsTruckInvoices, predefinedType, nextInvoiceNumber, taxMode, defaultCurrency, currencyOptions, initialExchangeRate, initialExchangeRates, exchangeRateApiUrl } = window.APP_DATA;
            ...
            const [formData, setFormData] = useState({
                ...
                tax_percentage: 18,
                shipping_charges: 0,
                order_type: supportsTruckInvoices ? (predefinedType || 'spare') : 'spare',
                currencies: (() => {
                    const isTruck = (predefinedType || 'spare') === 'truck';
                    if (isRoadmaster && isTruck) {
                        return [];
                    }
                ...
                primaryCurrency: (() => {
                    ...
                    return String(defaultCurrency || 'TZS').toUpperCase();
                })(),
                exchange_rates: (() => {
                    const seeded = { ...(initialExchangeRates || { TZS: '1.0000' }) };
                    const code = String(defaultCurrency || 'TZS').toUpperCase();
            ...
            const fetchBotExchangeRate = React.useCallback((code) => {
                ...
                if (currencyCode === 'TZS') {
                    setFormData(prev => ({
                        ...prev,
                        exchange_rates: { ...prev.exchange_rates, TZS: '1.0000' },
                    }));
                    return Promise.resolve();
                }
                const token = ++rateFetchTokenRef.current;
                setRateLoadingCodes((prev) => [...new Set([...prev, currencyCode])]);
                return fetch(exchangeRateApiUrl + '?currency=' + encodeURIComponent(currencyCode), { credentials: 'same-origin' })
                    .then((response) => response.json().catch(() => ({ ok: false, error: 'Invalid response from server.' })))
                    .then((data) => {
                        if (token !== rateFetchTokenRef.current) return;
                    ...
                    .catch(() => {
                        if (token !== rateFetchTokenRef.current) return;
                        if (!applyDefaultBotRate(currencyCode)) {
                ...
                if (saved) {
                    try {
                        ...
                        if (draft.formData) {
                            ...
                            const restored = Array.isArray(draft.formData.currencies) ? draft.formData.currencies : [];
                            restored.filter((code) => code && code !== 'TZS').forEach((code) => {
                                if (!draft.formData.exchange_rates?.[code]) {
                                    setTimeout(() => fetchBotExchangeRate(code), 0);
                                }
                            });
                        }
                ...
                if (field === 'product_id') {
                    ...
                    if (prod) {
                        ...
                        if (isRoadmaster && prod.item_type === 'vehicle' && formData.order_type !== 'truck') {
                            setFormData(prev => ({ ...prev, order_type: 'truck', currencies: [], primaryCurrency: '' }));
                            if (window.Swal) {
                                Swal.fire({
            ...
            const displayCurrencies = (() => {
                const raw = Array.isArray(formData.currencies) ? formData.currencies : [];
                if (!raw.length) {
                    return [];
                }
            ...
            const primaryCurrency = formData.primaryCurrency || displayCurrencies[0] || '';
            ...
            const moneyLabel = (amount) => {
                ...
                return displayCurrencies.map((code) => `${code} ${formatCurrency(convertAmount(amount, primaryCurrency, code))}`).join(' · ');
            };
            const nonTzsCurrencies = displayCurrencies.filter((code) => code !== 'TZS');
            const isTruckInvoice = supportsTruckInvoices && formData.order_type === 'truck';

            const handleSubmit = (e) => {
                if (isTruckInvoice && !hasSelectedCurrencies) {
                    e.preventDefault();
                    if (window.Swal) {
                        Swal.fire({
                            icon: 'warning',
                            title: 'Currency required',
                            text: 'Please select at least one currency before creating the truck invoice.',
                        });
                    } else {
                        alert('Please select at least one currency before creating the truck invoice.');
                    }
                }
            };
            const invoiceTypeLabel = isUltimate ? 'Invoice' : (isTruckInvoice ? 'Truck Invoice' : 'Spare Invoice');
...
`}</style>
                    <form method="POST" onSubmit={handleSubmit}>
                        ...
                        <input type="hidden" name="display_currencies" value={JSON.stringify(displayCurrencies)} />
                        <input type="hidden" name="currency_rates" value={JSON.stringify(formData.exchange_rates || { TZS: '1.0000' })} />
                        {!isRoadmaster ? (
                        ...
                        <div className="quote-header">
                            <div className="quote-title">
                                <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{invoiceTypeLabel}</h1>
                                <p className="text-sm text-slate-500 mt-1">Draft a new invoice and add items from the catalogue or manually.</p>
                                {isRoadmaster ? (
                                    <span className={`inv-type-badge ${isTruckInvoice ? 'inv-type-badge--truck' : 'inv-type-badge--spare'}`}>
                                        <i className={`fas ${isTruckInvoice ? 'fa-truck' : 'fa-cogs'}`}></i>
                                        {isTruckInvoice ? 'Truck layout' : 'Spare layout'}
                                    </span>
                                ) : null}
                            </div>
                            ...
                            <div className="quote-left">
                                <div className="quote-top-grid">
                                    <div className="quote-card">
                                        ...
                                        {isRoadmaster ? (
                                            <div className="quote-form-row">
                                                <label className="quote-form-label">Invoice type</label>
                                                <div>
                                                    <div className={`inv-type-summary ${isTruckInvoice ? 'inv-type-summary--truck' : 'inv-type-summary--spare'}`}>
                                                        <i className={`fas ${isTruckInvoice ? 'fa-truck' : 'fa-cogs'}`}></i>
                                                    ...
                                                    <select
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            if (nextType === 'truck') {
                                                                setFormData((prev) => ({ ...prev, order_type: nextType, currencies: [], primaryCurrency: '' }));
                                                            } else {
                                                                const code = String(defaultCurrency || 'TZS').toUpperCase();
                                                                setFormData((prev) => ({ ...prev, order_type: nextType, currencies: [code], primaryCurrency: code }));
                                                            }
                                                        }}
                                                        className="quote-select"
                                                    ...
                                                    <div className="inv-rate-hint" style={{ marginTop: '6px' }}>
                                                        Selected from the invoices list. You can change the layout here if needed.
                                                    </div>
                                                </div>
                                            </div>
                                        ) : null}
                                        <div className="quote-form-row">
                                            <label className="quote-form-label required">Currencies</label>
                                            <div>
                                                    ...
                                                    {displayCurrencies.map((code) => {
                                                        ...
                                                        return (
                                                            <span key={code} className={`inv-currency-chip${isPrimary ? ' is-primary' : ''}`}>
                                                                <img src={`${flagBase}${meta.flag}.png`} alt="" className="inv-currency-flag" />
                                                                <strong>{code}</strong>
                                                                <button
                                                                    type="button"
                                                                    className={isPrimary ? 'is-active' : ''}
                                                                    onClick={() => setPrimaryCurrency(code)}
                                                                    title="Use as billing currency"
                                                                >
                                                ...
                                                <div className={`inv-currency-picker${currencyMenuOpen ? ' is-open' : ''}`}>
                                                    <button
                                                        ...
                                                        onClick={(e) => { e.stopPropagation(); setCurrencyMenuOpen((open) => !open); }}
                                                    >
                                                        <span className="inv-currency-label">
                                                            <span className="code">Add / remove currencies</span>
                                                            <span className="name">{displayCurrencies.length} selected</span>
                                                        </span>
                                                    </button>
                                                    {currencyMenuOpen ? (
                                                            ...
                                                            {Object.entries(currencyOptions).map(([code, meta]) => {
                                                                ...
                                                                return (
                                                                    <button
                                                                        key={code}
                                                                        type="button"
                                                                        className={`inv-currency-option${isChecked ? ' is-checked' : ''}`}
                                                ...
                                                <div className="inv-rate-hint">Billing currency: <strong>{primaryCurrency || 'Not selected'}</strong>. Amounts are stored in the billing currency and shown in every selected currency on the invoice.</div>
                                            </div>
                                        </div>
                                        {nonTzsCurrencies.length > 0 ? (
                                        <div className="quote-form-row">
                                            <label className="quote-form-label">Exchange rates</label>
                                            <div>
                                                <div className="inv-rate-grid">
                                                    {nonTzsCurrencies.map((code) => (
                                                        <div className="inv-rate-row" key={code}>
                                                            ...
                                                            <input
                                                                ...
                                                                step="0.0001"
                                                                min="0"
                                                                value={formData.exchange_rates?.[code] || ''}
                                                ...
                                                <div className={`inv-rate-hint${rateLoadingCodes.length ? ' is-loading' : ''}`}>
                            ...
                            <aside className="quote-summary">
                                <div className="quote-card">
                                    <div className="quote-summary-heading">
                                        ...
                                        {isRoadmaster ? (
                                            <span className={`inv-type-badge ${isTruckInvoice ? 'inv-type-badge--truck' : 'inv-type-badge--spare'}`} style={{ marginTop: 0, marginLeft: '8px' }}>
                                                {isTruckInvoice ? 'Truck' : 'Spare'}
                                            </span>
                                        ) : null}
                                    </div>
                                    <div>
                                        <div className="summary-row"><span>{taxMode === 'inclusive' ? 'Subtotal (excl. tax)' : 'Subtotal'}</span><strong>{moneyLabel(totals.subtotal)}</strong></div>
                                        ...
                                        <div className="summary-row"><span>{taxMode === 'inclusive' ? 'Tax included (%)' : 'Tax (%)'}</span><div style={{display:'flex', gap:8, alignItems:'center'}}><input type="number" step="0.01" name="tax_percentage" value={formData.tax_percentage} onChange={e=>setFormData({ ...formData, tax_percentage: Math.max(0, Math.min(100, parseFloat(e.target.value) || 0)) })} className="summary-input"/><span style={{minWidth:80, textAlign:'right'}}>{moneyLabel(totals.taxAmt)}</span></div></div>
                                        ...
                                        <div className="summary-total">
                                            ...
                                            <span style={{ textAlign: 'right' }}>
                                                {hasSelectedCurrencies ? displayCurrencies.map((code) => (
                                                    <div key={code} className={code === primaryCurrency ? '' : 'inv-summary-equiv'}>
                                                        {code} {formatCurrency(convertAmount(totals.grandTotal, primaryCurrency, code))}
                                                    </div>
                                                )) : (
