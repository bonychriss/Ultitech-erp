...
try {
    if (function_exists('ensureSalesOrderMultiCurrencyColumns')) {
        ensureSalesOrderMultiCurrencyColumns();
    }
} catch (Throwable $e) {
    error_log('invoice view ensureSalesOrderMultiCurrencyColumns: ' . $e->getMessage());
}
...
if (isRoadmaster()) {
    ...
    $isTruckOrder = (isset($invoice['order_type']) && strtolower(trim((string) $invoice['order_type'])) === 'truck');
}

$invoiceDualCurrencyCtx = null;
if ($isRoadmaster && $isTruckOrder && function_exists('sales_invoice_dual_currency_context')) {
...
<head>
    ...
    <style>
        ...
        body.pdf-mode .layout-main-wrapper > aside,
        body.pdf-mode .sidebar,
        body.pdf-mode .employee-header,
        body.pdf-mode .control-panel,
        body.pdf-mode .action-bar,
        body.pdf-mode #chatbotLauncher,
        body.pdf-mode .truck-continued-hint {
            display: none !important;
        }

        body.pdf-mode .main-content,
        body.pdf-mode #invoice-content {
            width: 210mm !important;
            max-width: 210mm !important;
            margin: 0 auto !important;
            padding: 0 !important;
        ...
        @media print {
            ...
            .layout-main-wrapper,
            .layout-main-wrapper > .flex-grow-1,
            .min-vh-100,
            .main-content,
            #invoice-content {
                min-height: 0 !important;
            ...
            /* style.css hides non-voucher main-content children when printing */
            body .main-content > #invoice-content,
            body .main-content > #catalog-content {
                display: block !important;
            }

            body, .main-content { margin: 0 auto; padding: 0; background: white; }
            .sheet-container { 
                width: 210mm; 
                max-width: 210mm;
                margin: 0 auto; 
            ...
            /* Keep full A4 width — width:100% was shrinking fonts to fit */
            .spare-sheet,
            .truck-sheet,
            .truck-sheet-second,
            .doc-terms-sheet,
            ...
            .no-print, .control-panel, .action-bar, header, .sidebar, .employee-header, .layout-main-wrapper > aside, #chatbotLauncher, .chatbot-launcher, .mobile-footer, #notif-backdrop { display: none !important; }
...
<body>
    ...
    <script src="<?php echo htmlspecialchars(function_exists('app_url') ? app_url('/modules/sales/assets/js/document-pdf-capture.js') : '/modules/sales/assets/js/document-pdf-capture.js', ENT_QUOTES, 'UTF-8'); ?>"></script>
    <script>
        ...
        /** Invoice sheet nodes to rasterize (page 1 + terms page, etc.). */
        function invoicePdfCaptureElements() {
            if (window.SalesDocumentPdfCapture) {
                return window.SalesDocumentPdfCapture.captureElements('invoice-content');
            }
        ...
        async function invoicePdfElementToCanvas(el) {
            ...
            try {
                const opts = window.SalesDocumentPdfCapture
                    ? window.SalesDocumentPdfCapture.elementToCanvasOptions(el, {})
                    : {
                        scale: 2,
                        useCORS: true,
                        logging: false,
        ...
        function appendRasterCanvasToPdf(doc, canvas, jpegQuality, options) {
            if (window.SalesDocumentPdfCapture) {
                window.SalesDocumentPdfCapture.appendRasterCanvasToPdf(doc, canvas, jpegQuality, options || {});
                return;
            }
        ...
        async function appendInvoiceElementsToPdf(doc, elements, jpegQuality) {
            if (window.SalesDocumentPdfCapture) {
                await window.SalesDocumentPdfCapture.appendElementsToPdf(
                    doc,
                    elements,
                    jpegQuality,
                    invoicePdfElementToCanvas,
                    { marginMm: 5 }
                );
                return;
            }
        ...
        async function generateAndSendPdf(targetUrl) {
            ...
            try {
                ...
                if (catalogElement && catalogElement.offsetHeight > 0) {
                    ...
                    appendRasterCanvasToPdf(doc, catalogCanvas, 0.93, { startOnNewPage: true, marginMm: 5 });
                }
