...
import { formatQuotationKpiHeadline } from '../utils/quotationKpiTrace';

export default function QuotationKpiTraceModal({ trace, onClose }) {
  ...
  return createPortal(
    <div className="exp-desk-modal-backdrop" onClick={onClose} role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="qt-kpi-trace-title"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="exp-desk-modal-head">
          <div className="exp-desk-kpi-trace-head">
            <div>
              <h2 id="qt-kpi-trace-title" className="exp-desk-kpi-trace-title">{trace.title}</h2>
              <p className="exp-desk-kpi-trace-headline">{formatQuotationKpiHeadline(trace)}</p>
            </div>
          </div>
            ...
            <section className="exp-desk-kpi-trace-section exp-desk-kpi-trace-section--confirm">
              ...
              {trace.duration?.label && (
                <p className="qt-kpi-summary-duration">
                  <span className="qt-kpi-summary-duration-label">Quote period:</span>
                  {' '}
                  {trace.duration.label}
                </p>
              )}
                  ...
                  <div key={period.key} className="qt-kpi-summary-period">
                    ...
                    <div className="qt-kpi-summary-period-metrics">
                      <span className="qt-kpi-summary-period-count">
                        {period.countDisplay}
                        {' '}
                        {Number(period.count) === 1 ? 'quotation' : 'quotations'}
                      </span>
