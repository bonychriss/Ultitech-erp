...
try {
    if (!empty($order['id'])) {
        $invCheck = $salesDb->prepare('SELECT id FROM invoices WHERE order_id = ? LIMIT 1');
        $invCheck->execute([(int) $order['id']]);
        $hasLinkedInvoice = (int) ($invCheck->fetchColumn() ?: 0) > 0;
    }
} catch (Throwable $e) {
    $hasLinkedInvoice = false;
}
if (function_exists('sales_order_document_title_label')) {
    $doc_title = sales_order_document_title_label($orderStatusKey, $hasLinkedInvoice);
} else {
    $doc_title = in_array($orderStatusKey, ['draft', 'quotation', 'sent'], true) ? 'Quotation' : 'Sales Order';
}
$is_quote = ($doc_title === 'Quotation');
$filename_prefix = $is_quote ? 'Quotation_' : ($doc_title === 'Invoice' ? 'Invoice_' : 'Order_');
if ($doc_title === 'Quotation') {
    $sheetDateLabel = 'Quote Date';
    $sheetDueLabel = 'Valid until';
} elseif ($doc_title === 'Invoice') {
    $sheetDateLabel = 'Invoice Date';
    $sheetDueLabel = 'Due Date';
} else {
    $sheetDateLabel = 'Order Date';
    $sheetDueLabel = 'Valid until';
}
...
<body <?php if(!$auto_print) echo ''; ?>>
    ...
    <script>
        function notifySalesDocPdfStart() {
            var payload = { type: 'sales-doc-pdf-start' };
            try {
        ...
        function notifySalesDocPdf(ok, error) {
            var payload = { type: 'sales-doc-pdf', ok: !!ok, error: error || '' };
            try {
                if (window.opener && !window.opener.closed) {
                    window.opener.postMessage(payload, window.location.origin);
                }
        ...
        function downloadPDF() {
            if (typeof html2pdf === 'undefined') {
                notifySalesDocPdf(false, 'PDF library failed to load.');
                return;
            }
            notifySalesDocPdfStart();
            ...
            if (!element) {
                notifySalesDocPdf(false, 'Document content not found.');
                return;
            }
            ...
            html2pdf().set(opt).from(element).save().then(function(){
                ...
                notifySalesDocPdf(true, '');
            }).catch(function(err){
                document.body.classList.remove('pdf-mode');
                if(btn) btn.style.display = 'block';
                notifySalesDocPdf(false, (err && err.message) ? err.message : 'PDF generation failed.');
            });
        }
    ...
    <?php if(!$auto_print && empty($embedMode)): ?>
                    ...
                    <?php if ($page_idx === 0): ?>
                        ...
                        <div style="margin-top: 10px; margin-bottom: 10px; border: 1px solid #000; display: flex; text-align: left;">
                            <div style="flex: 1; padding: 4px 8px; border-right: 1px solid #ccc;">
                                <div style="font-weight: bold; font-size: 0.8rem;"><?php echo htmlspecialchars($sheetDateLabel); ?></div>
                            ...
                            <div style="flex: 1; padding: 4px 8px; border-right: 1px solid #ccc;">
                                <div style="font-weight: bold; font-size: 0.8rem;"><?php echo htmlspecialchars($sheetDueLabel); ?></div>
