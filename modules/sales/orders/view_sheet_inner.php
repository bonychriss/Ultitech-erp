...
$hasLinkedInvoice = !empty($hasLinkedInvoice);
$sheetTitleLabel = function_exists('sales_order_document_title_label')
    ? sales_order_document_title_label($orderStatusKey, $hasLinkedInvoice)
    : (($orderStatusKey === 'draft' || $orderStatusKey === 'quotation') ? 'Quotation' : 'Sales Order');
if ($sheetTitleLabel === 'Quotation') {
    $sheetDateLabel = 'Quote Date';
    $sheetDueLabel = 'Valid until';
} elseif ($sheetTitleLabel === 'Invoice') {
...
?>
<div class="sheet-container" style="font-family: <?= $docFontStack ?>; font-size: 13px;">
    <div class="sheet" style="display: flex; flex-direction: column; min-height: 297mm; font-family: inherit;">
        <div class="sheet-header-title quot-sheet-header" style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px;">
            <h1 class="sheet-title">
                <?php echo htmlspecialchars($sheetTitleLabel, ENT_QUOTES, 'UTF-8'); ?> #
            ...
            <div class="quot-company-block" style="text-align: right;">
                ...
                ?>
                <img src="<?php echo htmlspecialchars((string) $logoPath, ENT_QUOTES, 'UTF-8'); ?>" alt="Company Logo" crossorigin="anonymous" style="max-height: 80px; margin-bottom: 10px;" onerror="this.style.display='none'">
        ...
        <div style="display: flex; flex-direction: column; flex: 1;">
            ...
            <div class="date-info-bar" style="margin-top: 10px; margin-bottom: 10px; border: 1px solid #000; display: flex; text-align: left;">
                <div style="flex: 1; padding: 4px 8px; border-right: 1px solid #ccc;">
                    <div style="font-weight: bold; font-size: 0.72rem;"><?php echo htmlspecialchars($sheetDateLabel); ?></div>
                    <div style="font-size: 0.78rem;"><?php echo date('d/m/Y', strtotime($order['quote_date'])); ?></div>
                </div>
                <div style="flex: 1; padding: 4px 8px; border-right: 1px solid #ccc;">
                    <div style="font-weight: bold; font-size: 0.72rem;"><?php echo htmlspecialchars($sheetDueLabel); ?></div>
...
<?php
$salesDocTermsNumber = (string) (($displayOrderNumber ?? '') !== '' ? $displayOrderNumber : ($order['order_number'] ?? ''));
$salesDocTermsTypeLabel = $sheetTitleLabel ?? (function_exists('sales_order_document_title_label')
    ? sales_order_document_title_label($order['status'] ?? '', !empty($hasLinkedInvoice))
    : 'Sales Order');
