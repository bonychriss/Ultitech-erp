...
if (is_file(__DIR__ . '/../../../includes/invoice_gl_posting.php')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $paymentId = (int) $salesDb->lastInsertId();
        ...
        if (function_exists('invoice_gl_post_invoice_payment')) {
            $glPdo = ($balancesPdo instanceof PDO) ? $balancesPdo : $salesDb;
            invoice_gl_post_invoice_payment(
                $glPdo,
                (int) $invoice_id,
                $paymentId,
                (float) $amount,
