...
if ($type === 'receivables') {
    ...
    $sql = "SELECT i.*, c.company_name as party_name, DATEDIFF(CURRENT_DATE, i.invoice_date) as days_overdue 
            FROM invoices i 
            JOIN customers c ON i.customer_id = c.id 
            WHERE i.status NOT IN ('paid', 'cancelled') OR i.status = 'partial'";
} else {
    // Payables based on Payment Vouchers (that aren't paid yet)
    $sql = "SELECT v.id, v.total_amount as total, v.payee_name as party_name, DATEDIFF(CURRENT_DATE, v.date_created) as days_overdue 
            FROM payment_vouchers v 
            WHERE v.is_paid = 0 AND v.status != 'cancelled'"; 
}
