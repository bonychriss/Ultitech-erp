...
$stmt = $pdo->prepare("SELECT SUM(total_amount) FROM invoices WHERE status != 'cancelled' AND MONTH(invoice_date) = MONTH(CURRENT_DATE()) AND YEAR(invoice_date) = YEAR(CURRENT_DATE())");
$stmt->execute();
$monthlySales = $stmt->fetchColumn() ?: 0;

// 2. Total Expenses (This Month)
$stmt = $pdo->prepare("SELECT SUM(amount) FROM expenses_requests WHERE status = 'approved' AND MONTH(date) = MONTH(CURRENT_DATE()) AND YEAR(date) = YEAR(CURRENT_DATE())");
$stmt->execute();
$monthlyExpenses = $stmt->fetchColumn() ?: 0;

// 3. Outstanding Invoices (Receivables)
$stmt = $pdo->query("SELECT SUM(balance_due) FROM invoices WHERE status != 'cancelled'");
$outstandingReceivables = $stmt->fetchColumn() ?: 0;

// 4. Low Stock Items
$stmt = $pdo->query("SELECT COUNT(*) FROM products p JOIN stock s ON p.id = s.product_id WHERE s.quantity <= p.reorder_level");
...
<div class="page-wrapper">
    ...
    <div class="container">
        ...
        <div class="reports-grid">
            ...
            <a href="logistics.php" class="report-card">
                <div class="report-icon"><i class="fas fa-truck"></i></div>
                <div class="report-content">
                    <div class="report-title">Logistics Analytics</div>
                    <div class="report-desc">Delivery and dispatch performance tracking with route optimization.</div>
                </div>
            </a>
        </div>
    </div>
