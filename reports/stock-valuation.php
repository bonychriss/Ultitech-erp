...
// 1. Overall Valuation
$sql = "SELECT SUM(s.quantity * p.$costCol) as total_value, COUNT(*) as total_items 
        FROM products p 
        JOIN stock s ON p.id = s.product_id";
$overall = $pdo->query($sql)->fetch();
...
SELECT 
    ...
    (s.quantity * p.$costCol) as total_value
FROM products p
JOIN stock s ON p.id = s.product_id
WHERE s.quantity > 0
GROUP BY p.id, p.name, p.product_code, s.quantity, p.$costCol
