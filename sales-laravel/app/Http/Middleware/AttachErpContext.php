...
/**
 * Inject ERP session context (set by public_html/sales.php bridge).
 */
class AttachErpContext
{
    public function handle(Request $request, Closure $next): Response
    {
        $erp = $GLOBALS['ERP_SALES_CONTEXT'] ?? null;
        if (!is_array($erp) || empty($erp['user_id'])) {
            abort(401, 'ERP login required. Open this page from the ERP Sales menu.');
        }
