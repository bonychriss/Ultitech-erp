...
$companyName = ($val = getCompanySetting('company_name')) && trim($val) !== '' ? $val : (defined('COMPANY_NAME') ? COMPANY_NAME : 'Ultimate General Trading Company');
$companyAddress = ($val = getCompanySetting('company_address')) && trim($val) !== '' ? $val : "Plot 123, Standard Street, Dar es Salaam\nP.O.BOX 7800";
$companyPhone = ($val = getCompanySetting('company_phone')) && trim($val) !== '' ? $val : '+255 123 456 789';
$companyEmail = ($val = getCompanySetting('company_email')) && trim($val) !== '' ? $val : 'info@company.com';
$companyTin = getCompanySetting('company_tin') ?: '';
$companyVat = getCompanySetting('company_vat') ?: '';
$companyLocation = getCompanySetting('company_location') ?: '';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media print {
            ...
            .totals-area, .totals-table, .chatter, .note-box, .sheet-header-title, .form-grid {
                page-break-inside: avoid !important;
                break-inside: avoid !important;
            }
        }
    ...
    <div class="sheet-container">
        <div class="sheet">
            ...
            <div class="sheet-header-title" style="display: flex; justify-content: space-between; align-items: flex-start;">
                ...
                <div style="text-align: right; display: flex; flex-direction: column; align-items: flex-end;">
                    <?php if (!empty($logoUrl)): ?>
                        <img src="<?= htmlspecialchars($logoUrl) ?>" alt="Logo" style="height: 60px; margin-bottom: 10px;" onerror="this.style.display='none'">
                    <?php endif; ?>
                    <div style="font-weight: bold; font-size: 1rem; color: #111; text-transform: uppercase;"><?= htmlspecialchars($companyName) ?></div>
                    <div style="font-size: 0.9rem; color: #333; white-space: pre-wrap;"><?= htmlspecialchars($companyAddress) ?><?php if (!empty($companyLocation)): ?>, <?= htmlspecialchars($companyLocation) ?><?php endif; ?></div>
                    <?php if (!empty($companyPhone)): ?>
                        <div style="font-size: 0.9rem; color: #333;">Phone: <?= htmlspecialchars($companyPhone) ?></div>
                    <?php endif; ?>
                    <?php if (!empty($companyEmail)): ?>
                        <div style="font-size: 0.9rem; color: #333;">Email: <?= htmlspecialchars($companyEmail) ?></div>
                    <?php endif; ?>
                    <?php if (!empty($companyTin)): ?>
                        <div style="font-size: 0.9rem; color: #333;">TIN: <?= htmlspecialchars($companyTin) ?></div>
                    <?php endif; ?>
                    <?php if (!empty($companyVat)): ?>
                        <div style="font-size: 0.9rem; color: #333;">VAT: <?= htmlspecialchars($companyVat) ?></div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="form-grid" style="margin-bottom: 20px;">
                <div class="left-col" style="width: 100%;">
                    <div class="form-group">
                        ...
                        <div class="form-value" style="font-size: 1rem; line-height: 1.4; color: #333;">
                            <strong style="font-size: 1.25rem; color: #000; font-weight: bold;"><?= htmlspecialchars($invoice['customer_name']) ?></strong><br>
                            <?php if(!empty($invoice['customer_address'])): ?>
                                <span><?= nl2br(htmlspecialchars($invoice['customer_address'])) ?></span><br>
                            <?php endif; ?>
                            <?php if(!empty($invoice['customer_email'])): ?>
                                <span><?= htmlspecialchars($invoice['customer_email']) ?></span><br>
                            <?php endif; ?>
                            <?php if(!empty($invoice['customer_phone'])): ?>
                                <span><?= htmlspecialchars($invoice['customer_phone']) ?></span><br>
                            <?php endif; ?>
                            <?php 
            ...
            <div style="margin-top: 10px; margin-bottom: 30px; border: 1px solid #000; display: flex; text-align: left;">
                ...
                <?php if (isset($invoice['lead_time']) && $invoice['lead_time'] !== null && $invoice['lead_time'] !== ''): ?>
                <div style="flex: 1; padding: 2px;">
                    <div style="font-weight: bold; font-size: 0.8rem;">Lead Time</div>
                    <div style="font-size: 0.85rem;"><?= htmlspecialchars($invoice['lead_time']) ?> Days</div>
                </div>
                <?php endif; ?>
                <div style="flex: 1; padding: 2px;">
                    <div style="font-weight: bold; font-size: 0.8rem;">Salesperson</div>
                    <div style="font-size: 0.85rem;"><?= htmlspecialchars($invoice['created_by_name'] ?? 'System User') ?></div>
                </div>
            </div>
