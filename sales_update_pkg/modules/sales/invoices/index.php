...
$salesModuleContext = (isset($_GET['module']) && strtolower(trim((string) $_GET['module'])) === 'sales');
...
<body class="page-sales-invoices">
    ...
    <script>
        window.APP_DATA = {
            ...
            salesModuleContext: <?= sales_json_script($salesModuleContext) ?>,
    ...
    <script>
        window.addEventListener('error', function(e) {
            document.getElementById('react-root').innerHTML = '<div style="color:red; padding: 20px;">' + e.message + '<br>' + e.filename + ':' + e.lineno + '</div>';
        });
    </script>

    <script type="text/babel">
        const { useState, useMemo, useEffect } = React;
        const { invoices: initialInvoices, currentUserId, isRoadmaster, showOrderTypeColumn, invoicesShellLayout, salesModuleContext, defaultCurrency: appInvCurrency } = window.APP_DATA;
        const useRmShell = invoicesShellLayout === true;
        const forceListView = salesModuleContext === true;
        ...
        function getStatusConfig(status) {
            ...
            return configs[st] || { color: 'bg-slate-50 text-slate-600 border-slate-100', label: status || 'Ã¢â‚¬â€' };
        }
        ...
        function sortInvoicesByNewest(list) {
        ...
        function InvoicesApp() {
            ...
            useEffect(() => {
                if (forceListView) return;
                try { localStorage.setItem('sales_invoices_view_mode', viewMode); } catch (e) {}
            }, [viewMode]);

            const effectiveViewMode = forceListView ? 'list' : viewMode;

            useEffect(() => { setPage(1); }, [search, statusFilter, myInvoicesOnly, effectiveViewMode]);
            ...
            const filteredInvoices = useMemo(() => {
                ...
                return sortInvoicesByNewest(list);
            }, [invoices, search, statusFilter, myInvoicesOnly, currentUserId]);

            const pageSize = forceListView
                ? Math.max(filteredInvoices.length, 1)
                : (useRmShell ? 10 : PAGE_SIZE);
            ...
            function RowCells({ inv }) {
                ...
                return (
                    <>
                        ...
                        <td className={'px-3 py-2.5 truncate max-w-[200px] ' + (useRmShell ? 'py-3 text-base font-bold uppercase tracking-tight text-gray-900' : 'text-base text-gray-900 font-medium')} title={inv.customer_name || ''}>{inv.customer_name || 'Ã¢â‚¬â€'}</td>
                        <td className={'px-3 py-2.5 whitespace-nowrap ' + (useRmShell ? 'py-3' : '')}>
                            <div className="flex items-center gap-2">
                                <SalespersonAvatar name={inv.salesperson} />
                                <span className="text-base text-gray-700 truncate max-w-[130px]">{inv.salesperson || 'Ã¢â‚¬â€'}</span>
                            </div>
                        </td>
                            ...
                            {pagedInvoices.map((inv) => {
                                ...
                                return (
                                    <div key={inv.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow">
                                        ...
                                        <p className="text-base font-medium text-gray-800 truncate">{inv.customer_name || 'Ã¢â‚¬â€'}</p>
                                        <div className="flex items-center gap-2 mt-2">
                                            <SalespersonAvatar name={inv.salesperson} size="sm" />
                                            <span className="text-sm text-gray-600 truncate">{inv.salesperson || 'Ã¢â‚¬â€'}</span>
                                        </div>
                                        ...
                                        <p className="text-sm text-gray-500 mt-1">Due {formatDate(inv.due_date)} Ã‚Â· Bal {formatCurrency(inv.balance_due)}</p>
                                ...
                                return (
                                    <div key={col.key} className="w-72 shrink-0 bg-gray-100/90 rounded-lg shadow-sm flex flex-col max-h-[72vh]">
                                        ...
                                        <div className="p-2 space-y-2 overflow-y-auto flex-1">
                                            {items.length === 0 ? (
                                                <p className="text-sm text-gray-400 text-center py-6">Ã¢â‚¬â€</p>
                                            ) : (
                                                items.map((inv) => {
                                                    const config = getStatusConfig(inv.status);
                                                    return (
                                                        <div key={inv.id} className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm hover:shadow-md transition-shadow">
                                                            ...
                                                            <p className="text-sm text-gray-600 truncate mt-1">{inv.customer_name || 'Ã¢â‚¬â€'}</p>
                                                            <div className="mt-1.5 flex items-center gap-2 min-w-0">
                                                                <SalespersonAvatar name={inv.salesperson} size="sm" />
                                                                <span className="text-xs text-gray-600 truncate font-medium">{inv.salesperson || 'Ã¢â‚¬â€'}</span>
                                                            </div>
                                                            <p className="text-sm text-gray-500 mt-1">{formatCurrency(inv.total_amount)} Ã‚Â· Bal {formatCurrency(inv.balance_due)}</p>
                ...
                <div className="flex flex-wrap items-center gap-3 p-4 border-b border-gray-50 bg-white">
                    <div className="relative flex-1 min-w-[200px] max-w-md">
                        ...
                        <input
                            ...
                            placeholder="Search number, order, customer, salespersonÃ¢â‚¬Â¦"
                    ...
                    {forceListView ? (
                        <span className="text-sm text-gray-600 tabular-nums whitespace-nowrap px-1">
            ...
            const invFooter = (filteredInvoices.length > 0 && effectiveViewMode === 'list' && !forceListView) ? (
            ...
            ) : (filteredInvoices.length > 0 && effectiveViewMode === 'list' && forceListView) ? (
                <div className="px-4 py-3 border-t border-gray-100 bg-white text-sm text-gray-600">
                    Showing all <span className="font-semibold text-gray-800">{filteredInvoices.length}</span> invoice{filteredInvoices.length === 1 ? '' : 's'}
                </div>
            ) : null;
                ...
                <div className="max-w-full mx-auto animate-fade-in">
                    <div className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
                        <div className="px-4 py-3 flex flex-wrap items-center gap-3 border-b border-gray-100">
                            ...
                            {forceListView ? (
                                <span className="text-base text-gray-600 tabular-nums whitespace-nowrap">
                                    {filteredInvoices.length} invoice{filteredInvoices.length === 1 ? '' : 's'}
                                </span>
                            ) : (
