...
<style>
    ...
    /* Bill To Bar */
    .spare-bill-bar {
        border: 2px solid <?= $isRoadmaster ? $brandYellow : '#000' ?>;
        display: flex;
        padding: 5px 10px;
        font-size: 13px;
        font-weight: 700;
    ...
    .row-total {
        border-top: 2px solid <?= $isRoadmaster ? $brandYellow : '#000' ?>;

        font-weight: 800 !important;
        color: <?= $isRoadmaster ? $brandNavy : '#000' ?>;
    }
...
<div class="spare-sheet">
    ...
    <div class="spare-header">
        <div class="spare-header-left">
            <div class="spare-logo-container">
                ...
                <?php endif; ?>
                <?php if ($layout != 2): ?>
                    <h1 class="spare-company-name-main">ROADMASTER</h1>
                    <div class="spare-company-name-sub">SPARES LIMITED</div>
                    <div class="spare-tagline"><?= $isRoadmaster ? 'YOUR TRUCK SPARES PARTNER' : 'PARTS THAT LAST, BACKED BY MASTERS' ?></div>
                <?php endif; ?>
            </div>
            ...
            <table class="spare-totals-table">
                <?php if (!in_array((int) $layout, [1, 2, 3], true)): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars((string) $currency) ?> <?= number_format((float) $invoice['total_amount'] - (float) ($invoice['tax_amount'] ?? 0), 2) ?></td>
                    </tr>
                    <tr>
                        <td class="spare-label-col">Taxes:</td>
                        <td><?= htmlspecialchars((string) $currency) ?> <?= number_format((float) ($invoice['tax_amount'] ?? 0), 2) ?></td>
                    </tr>
                <?php endif; ?>
                <tr class="row-total">
                    <td class="spare-label-col">Total:</td>
                    <td><?= htmlspecialchars((string) $currency) ?> <?= number_format((float) ($invoice['total_amount'] ?? 0), 2) ?></td>
                </tr>
    ...
    <div class="spare-bottom-info avoid-break">
        ...
        <div class="spare-thank-you">
            <strong style="color: #000;"><?= htmlspecialchars($company_settings['spare_thanks_note'] ?? ($isRoadmaster ? 'Thank you for Choosing Roadmaster' : 'Thank you for your business')) ?></strong>
