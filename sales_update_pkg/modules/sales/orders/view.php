...
if (!empty($order['created_by'])) {
    $signatureCandidateIds[] = (int) $order['created_by'];
}
if (!empty($_SESSION['user_id'])) {
    $signatureCandidateIds[] = (int) $_SESSION['user_id'];
}
$signatureCandidateIds = array_values(array_unique(array_filter($signatureCandidateIds)));

foreach ($signatureCandidateIds as $signatureUserId) {
    $rawSignaturePath = function_exists('getUserSignaturePathById')
        ? getUserSignaturePathById($signatureUserId)
        : null;
    $resolvedSignatureUrl = ($rawSignaturePath && function_exists('mediaUrlFromPath'))
        ? mediaUrlFromPath($rawSignaturePath)
        : '';
...
if ($signatureUrl === '' && !empty($order['salesperson']) && function_exists('getUserSignaturePathByName')) {
    $rawSignaturePath = getUserSignaturePathByName((string) $order['salesperson']);
    $signatureUrl = ($rawSignaturePath && function_exists('mediaUrlFromPath'))
        ? mediaUrlFromPath($rawSignaturePath)
        : '';
}
...
<head>
    ...
    <style>
        ...
        body {
            background: #f0f2f5;
            font-family: 'Arima', -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif;
            color: #374151;
            font-size: 0.9rem;
        }
    ...
    <div class="main-content p-0">
        ...
        <div class="control-panel" style="display: flex; justify-content: space-between; align-items: center; padding: 10px 30px; background: #f8f9fa; border-bottom: 1px solid var(--odoo-border);">
            <div class="breadcrumb">
                ...
                <span class="sep text-muted mx-2">/</span>
                <span class="active"><?php echo htmlspecialchars($displayOrderNumber); ?></span>
            </div>
            ...
            <div class="header-inner" style="display: flex; justify-content: flex-start; align-items: center; width: 100%;">
                <div class="btn-group-custom action-bar-desktop-only">
                    ...
                    <?php if ($order['status'] === 'confirmed'): ?>
                        <a href="../invoices/create.php?order_id=<?php echo (int) $order['id']; ?>" class="btn-custom btn-primary-custom" title="Create Invoice" style="background:#008784; border-color:#008784;">
                ...
                <details class="order-actions-mobile action-bar-mobile-only" id="invoiceActionsMobile">
                    ...
                    <div class="order-actions-mobile-panel">
                        ...
                        <?php endif; ?>
                        <?php if ($order['status'] === 'confirmed'): ?>
                        <a class="order-actions-mobile-item fw-semibold" style="color:#008784;" data-close-invoice-actions="1" href="../invoices/create.php?order_id=<?php echo (int) $order['id']; ?>">
                            <i class="fas fa-file-invoice-dollar"></i> Create Invoice
                        </a>
                        <?php endif; ?>
        ...
        <?php 
        if ($isRoadmaster) {
            // Roadmaster specialized layouts
            $layoutPath = __DIR__ . '/../layouts/roadmaster/' . ($isTruckOrder ? 'truck-invoice-layout-inner.php' : 'spare-invoice-layout-inner.php');
            if (file_exists($layoutPath)) {
                // Map $order to $invoice for layout compatibility
                $invoice = $order;
                $currency = $order['currency'] ?? ($company_settings['default_currency'] ?? 'TZS');
                include $layoutPath;
            } else {
                include __DIR__ . '/view_sheet_inner.php'; // Fallback
            }
        } else {
