...
if (strpos(str_replace('\\', '/', $script_name), '/attendance/') !== false) {
    ...
    } elseif (($_GET['module'] ?? '') === 'attendance' || strpos($attPath, '/attendance/index.php') !== false) {
...
if ($active_module === 'analytics') {
    ...
    addItem($menuItems, 'smart_report', 'AI Smart Report', 'file-earmark-bar-graph', $prefix . 'modules/analytics/smart_report.php?module=analytics');
} else {
if (stripos($path_to_check, '/deliveries/') !== false) {
    addItem($menuItems, 'quick-delivery-note', 'Delivery Note', 'file-earmark-plus', $prefix . 'deliveries/create_delivery_note.php?module=deliveries');
}
...
switch ($active_module) {
    case 'stocks':
        ...
        addItem($menuItems, 'analytics', 'Analytics', 'graph-up', $prefix . 'stock/modules/analytics/index.php');
        ...
        addItem($menuItems, 'reports', 'Reports', 'file-text', $prefix . 'stock/modules/reports/stock.php');
        addItem($menuItems, 'settings', 'Settings', 'gear', $prefix . 'stock/settings.php');
        break;
    ...
    case 'accounting':
        ...
        addParentItem($menuItems, 'balances-parent', 'Balances', 'scale-balanced', [
            ...
            [
                'id' => 'balances-new-account',
                'label' => 'New Account',
                'icon' => 'plus-circle',
                'path' => $prefix . 'modules/balances/coa_create.php?module=balances',
            ],
            [
                'id' => 'balances-categories',
                'label' => 'Account Categories',
                'icon' => 'folder-plus',
                'path' => $prefix . 'modules/balances/category_create.php?module=balances',
            ],
            [
        ...
        if ($is_admin) {
            $revenueChildren[] = [
                ...
                'label' => 'Payments List',
                'icon' => 'credit-card',
                'path' => $prefix . 'revenue_payments.php?module=revenue',
            ];
        }
        addParentItem($menuItems, 'revenue-parent', 'Revenue', 'coins', $revenueChildren);
    ...
    case 'balances':
        ...
        addParentItem($menuItems, 'account-parent', 'Account', 'wallet2', [
            [
                'id' => 'accounts',
                'label' => 'All Accounts',
                'icon' => 'wallet2',
    ...
    case 'deliveries':
        ...
        addItem($menuItems, 'trips', 'All Trips', 'map', $prefix . 'deliveries/trips.php?module=deliveries'); 
        addItem($menuItems, 'new-request', 'New Delivery', 'plus-circle', $prefix . 'deliveries/create_delivery.php?module=deliveries');
        addItem($menuItems, 'new-note', 'Create Delivery Note', 'file-earmark-plus', $prefix . 'deliveries/create_delivery_note.php?module=deliveries');
        addItem($menuItems, 'driver', 'Driver Portal', 'person-badge', $prefix . 'deliveries/Driver.php?module=deliveries');
        ...
        addItem($menuItems, 'dispatch', 'Dispatch', 'truck', $prefix . 'dispatch/index.php?module=dispatch');
        break;
    ...
    case 'voucher':
        ...
        addItem($menuItems, 'user-manual', 'User Manual', 'book', $prefix . 'employee/user-manual.php?module=voucher');
        ...
        if ($is_admin) {
            ...
            addItem($menuItems, 'create', 'Create Voucher', 'file-plus', $prefix . 'employee/create-voucher.php?module=voucher');
            addItem($menuItems, 'create-payee', 'Create Payee', 'person-plus', $prefix . 'employee/create-voucher.php?module=voucher&action=create_payee');
            addItem($menuItems, 'view', 'View Vouchers', 'ticket-detailed', $prefix . 'employee/my-vouchers.php?module=voucher');
    ...
    case 'attendance':
        if ($is_admin) {
            ...
            addItem($menuItems, 'sign', 'Sign In/Out', 'clock', $prefix . 'attendance/index.php?module=attendance');
    ...
    case 'tasks':
        $isWeeklyTasksAiPage = ($current_page === 'ai_assistant.php')
            && (strpos(str_replace('\\', '/', $script_name), '/weekly_tasks/') !== false);
        addItem($menuItems, 'ai-assistant', 'AI Assistant', 'stars', $prefix . 'weekly_tasks/ai_assistant.php?module=tasks');
        if (!$isWeeklyTasksAiPage) {
    ...
    case 'outstanding':
     ...
     case 'petty_cash':
        ...
        addItem($menuItems, 'dashboard', 'Dashboard', 'wallet', $prefix . 'erp/petty-cash/index.php?module=petty_cash');
        addItem($menuItems, 'new', 'New Request', 'file-earmark-plus', $prefix . 'erp/petty-cash/replenishment.php?module=petty_cash');
        addItem($menuItems, 'voucher', 'New Voucher', 'ticket-perforated', $prefix . 'erp/petty-cash/create-voucher.php?module=petty_cash');
        ...
        addItem($menuItems, 'reports', 'Reports', 'file-bar-graph', $prefix . 'erp/petty-cash/reports.php?module=petty_cash');
        break;
     ...
     case 'revenue':
        ...
        addParentItem($menuItems, 'revenue-parent', 'Revenue', 'list-ul', $revenueChildrenCase);
        addParentItem($menuItems, 'journal-parent', 'Journal', 'journal-text', [
            [
                'id' => 'journal-entries',
                'label' => 'Journal Entries',
     ...
     case 'expenses':
        ...
        addItem($menuItems, 'dashboard', 'Expenses', 'bar-chart', $prefix . 'modules/expenses/index.php?module=expenses');
    ...
    case 'todo':
        ...
        addItem($menuItems, 'reports', 'Reports', 'bar-chart-line', $prefix . 'employee/attendance-analytics.php?module=attendance');
        break;
    ...
    default:
        ...
        addItem($menuItems, 'user-manual', 'User Manual', 'book', $prefix . 'employee/user-manual.php');
        if ($is_admin) {
...
if (!function_exists('isSidebarActive')) {
    function isSidebarActive($path, $current_page) {
        ...
        if ($linkQuery !== '') {
            if ($path_parsed === '' || $path_parsed !== $curPath) {
                return '';
            }
        ...
        if ($path_parsed === $curPath || $path === $current_uri) {
            return 'active';
        }
...
<style>
    ...
    .layout-main-wrapper {
        ...
        margin: 0 !important;
        padding: 0 !important;
        gap: 4px !important; /* Added gap between sidebar and content */
        min-height: 100vh;
    }
...
<nav id="native-sidebar" class="<?= htmlspecialchars($current_theme) ?>">
    ...
    <div class="d-flex align-items-center mb-3 mb-md-0 me-md-auto text-decoration-none justify-content-between w-100 sidebar-header">
        ...
        <i class="bi bi-list fs-4 cursor-pointer sidebar-toggle-icon" onclick="toggleSidebarCollapse()" style="color: var(--sidebar-text); cursor: pointer; display: none;"></i>
        
        <!-- Standard Toggle Button (Hidden in collapsed) -->
        <button class="btn btn-sm btn-link text-white d-none d-lg-block p-0 sidebar-collapse-btn" onclick="toggleSidebarCollapse()">
            <i class="bi bi-list fs-4" style="color: var(--sidebar-text)"></i>
        </button>
    </div>
    ...
    <ul class="nav nav-pills flex-column mb-auto">
        <?php foreach ($menuItems as $item):
            ...
            $icon = $item['icon'];
            // Icon mapping fixes
            if ($icon == 'home') $icon = 'house';
            if ($icon == 'users') $icon = 'people';
            if ($icon == 'user') $icon = 'person';
            if ($icon == 'shopping-bag') $icon = 'bag';
            if ($icon == 'document-text') $icon = 'file-text';
            if ($icon == 'chart-bar') $icon = 'bar-chart';
            if ($icon == 'chart-pie') $icon = 'pie-chart';
            if ($icon == 'cog') $icon = 'gear';
            if ($icon == 'grid') $icon = 'grid';
            if ($icon == 'cpu') $icon = 'cpu';
        ...
        ?>
            <li class="nav-item">
                <a href="<?= $hasChildren ? '#' : $item['path'] ?>" class="nav-link d-flex align-items-center w-100 <?= $isActive ?> <?= htmlspecialchars((string) ($item['linkClass'] ?? '')) ?> <?= ($item['id'] === 'export' ? 'btn-download-special' : '') ?><?= $hasChildren ? ' sidebar-parent-toggle' : '' ?><?= ($hasChildren && $isActive === 'active') ? ' is-open' : '' ?>" <?= $hasChildren ? 'onclick="toggleSidebarSubmenu(this); return false;"' : $attr ?> title="<?= htmlspecialchars($item['label']) ?>">
                        ...
                        ?>
                            ...
                            <li class="nav-item<?= $nestedChildren !== [] ? ' sidebar-nested-group' : '' ?>">
                                ...
                                <a href="#"
                                   ...
                                   title="<?= htmlspecialchars((string) $child['label']) ?>">
                                    <i class="bi bi-<?= htmlspecialchars((string) $child['icon']) ?>"></i>
                                ...
                                <?php else: ?>
                                <a href="<?= htmlspecialchars((string) ($child['path'] ?? '#')) ?>" class="nav-link <?= $childActive ?>" title="<?= htmlspecialchars((string) $child['label']) ?>">
                                    <i class="bi bi-<?= htmlspecialchars((string) $child['icon']) ?>"></i>
                                    <span class="sidebar-text"><?= htmlspecialchars((string) $child['label']) ?></span>
                                </a>
                                <?php endif; ?>
                            </li>
                        <?php endforeach; ?>
        ...
        <li class="nav-item">
             <a href="<?= $prefix ?>employee/user-manual.php" class="nav-link" title="User Manual">
                <i class="bi bi-book"></i>
                <span class="sidebar-text">User Manual</span>
            </a>
        </li>
        <?php endif; ?>
    </ul>
