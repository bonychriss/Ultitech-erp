...
try {
    $countSql = "SELECT COUNT(*) FROM (
        SELECT p.id FROM products p {$whereClause} GROUP BY p.id
    ) stock_prod_count";
...
// Missing photos = products with no main_image / gallery filename (SQL only — no disk flood).
...
try {
    $missWhere = $hasProductImages
        ...
        : "WHERE TRIM(IFNULL(p.main_image, '')) = ''";
    $missingImagesCount = (int) ($pdo->query("SELECT COUNT(*) FROM products p {$missWhere}")->fetchColumn() ?: 0);
