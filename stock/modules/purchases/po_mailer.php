...
function sendPOStatusEmail($purchase_id, $event, $pdo) {
    ...
    $portalUrl = "$protocol://$host/staff/stock/modules/purchases/supplier_response.php?token=" . $po['public_token'];
    ...
    // 4. Send Email
    if (!empty($recipient)) {
        if (sendEmail($recipient, $subject, $body, true, [], 'purchases')) { // true = isHtml
            // Log it
