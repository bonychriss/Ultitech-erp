        ...
        foreach ($preview_data as $row) {
            ...
            try {
                 $stmt = $pdo->prepare("INSERT INTO shipments (
                    shipment_number, supplier_id, contact_number, invoice_number, tracking_number, 
                ...
                // Generate a shipment number (e.g. SHP-YYYYMMDD-XXXX)
                $shipment_number = 'SHP-' . date('Ymd') . '-' . strtoupper(substr(uniqid(), -4));
                
                $stmt->execute([
                    $shipment_number, $supp_id, $row['contact'], $row['invoice_number'], $row['tracking_number'],
