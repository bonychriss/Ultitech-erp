...
try {
    ...
    if (!in_array('expected_arrival_date', $cols, true)) {
        $pdo->exec("ALTER TABLE stocks_purchase_orders ADD COLUMN expected_arrival_date DATE NULL AFTER exchange_rate");
    }
    if (!in_array('shipment_method', $cols, true)) {
        $pdo->exec("ALTER TABLE stocks_purchase_orders ADD COLUMN shipment_method VARCHAR(40) NULL AFTER expected_arrival_date");
    }
} catch (Exception $e) {
...
// Approved stock-purchase vouchers available for PO linking
...
if (isset($_POST['exchange_rate'])) {
    $manualRate = (float) $_POST['exchange_rate'];
    if ($manualRate >= 0) {
        $selectedRate = $manualRate;
    }
}
$rate = $selectedRate;
...
$shipmentMethodOptions = ['FOB', 'CIF', 'DDP', 'AIR FREIGHT', 'SEA FREIGHT'];
$selectedPurchaseOrderDate = (string) ($_POST['purchase_order_date'] ?? ($cloned_po['purchase_date'] ?? date('Y-m-d')));
if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $selectedPurchaseOrderDate) !== 1) {
    $selectedPurchaseOrderDate = date('Y-m-d');
}
$selectedExpectedArrivalDate = (string) ($_POST['expected_arrival_date'] ?? ($cloned_po['expected_arrival_date'] ?? ''));
$selectedShipmentMethod = strtoupper(trim((string) ($_POST['shipment_method'] ?? ($cloned_po['shipment_method'] ?? ''))));
if (!in_array($selectedShipmentMethod, $shipmentMethodOptions, true)) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $linkedVoucher = null;
    $purchase_type = $_POST['purchase_type'] ?? 'import';
    ...
    $manualRate = isset($_POST['exchange_rate']) ? (float) $_POST['exchange_rate'] : 0;
    ...
    $expected_arrival_date_raw = trim((string) ($_POST['expected_arrival_date'] ?? ''));
    $expected_arrival_date = (preg_match('/^\d{4}-\d{2}-\d{2}$/', $expected_arrival_date_raw) === 1) ? $expected_arrival_date_raw : null;
    $shipment_method = strtoupper(clean_input($_POST['shipment_method'] ?? ''));
    if (!in_array($shipment_method, $shipmentMethodOptions, true)) {
        $shipment_method = '';
    }
    ...
    // Resolve supplier from a free-form payee label when picker sets a temporary value.
    ...
    if (!$suppliersAvailable || empty($suppliers)) {
        ...
        $error = "Please enter Purchase Order Date.";
    } elseif ($purchase_type === 'import' && $shipment_method === '') {
        $error = "Please select Shipment Method / Incoterms.";
    } else {
        // Proceed...
        try {
            ...
            for ($i = 0; $i < count($product_ids); $i++) {
                ...
                // Convert BACK to USD for storage (supplier-link draft allows $0 placeholder until supplier quotes)
            ...
            // 2. Insert Header using the live stock PO schema
            ...
            $insertCols = ['po_number', 'supplier_id', 'purchase_type', 'supplier_invoice_no', 'status', 'procurement_workflow', 'created_by', 'created_at'];
            $insertVals = [
                $po_number,
                $supplier_id,
                $purchase_type,
            ...
            if (in_array('currency', $poCols, true)) {
                $insertCols[] = 'currency';
                $insertVals[] = $selectedCurrencyCode;
            }
            if (in_array('exchange_rate', $poCols, true)) {
                $insertCols[] = 'exchange_rate';
                $insertVals[] = $rate;
            }
            $purchaseDateColumn = pickExistingColumn($poCols, ['purchase_date', 'order_date', 'po_date']);
            if ($purchaseDateColumn !== null) {
                $insertCols[] = $purchaseDateColumn;
                $insertVals[] = $purchase_order_date;
            }
            if (in_array('expected_arrival_date', $poCols, true)) {
                $insertCols[] = 'expected_arrival_date';
                $insertVals[] = $expected_arrival_date;
            }
            if (in_array('shipment_method', $poCols, true)) {
                $insertCols[] = 'shipment_method';
                $insertVals[] = ($shipment_method !== '' ? $shipment_method : null);
            }
            if (in_array('payment_voucher_id', $poCols, true)) {
                $insertCols[] = 'payment_voucher_id';
                $insertVals[] = ($payment_voucher_id > 0 ? $payment_voucher_id : null);
            }
            if (in_array('payment_voucher_ids', $poCols, true)) {
                $insertCols[] = 'payment_voucher_ids';
                $insertVals[] = !empty($payment_voucher_ids) ? json_encode($payment_voucher_ids) : null;
            }
            $placeholders = implode(', ', array_fill(0, count($insertCols) - 1, '?')) . ', NOW()';
            $stmt = $pdo->prepare('INSERT INTO stocks_purchase_orders (' . implode(', ', $insertCols) . ') VALUES (' . $placeholders . ')');
...
/* ---- Screenshot-matching compact layout overrides ---- */
...
.po-card.card-supplier .po-card-header-icon {
    background: #2563eb !important;
}
.po-card.card-items .po-card-header-icon {
    background: #16a34a !important;
}
.po-card.card-info .po-card-header-icon {
    background: #d97706 !important;
}
.po-card.card-info .po-card-body {
    padding-top: 8px !important;
    padding-bottom: 8px !important;
}
.po-card.card-info .mb-4 {
    margin-bottom: 10px !important;
}
.po-card.card-info textarea.form-control {
...
.tax-input-wrapper .text-muted.small {
...
/* Top toolbar text sizing */
main.main-content.mov-shell .sticky.top-0 a {
    font-size: 16px !important;
}
main.main-content.mov-shell .sticky.top-0 h1 {
    font-size: 23px !important;
}
main.main-content.mov-shell .sticky.top-0 .text-gray-600 {
    font-size: 16px !important;
}
main.main-content.mov-shell .po-toolbar-main {
    padding-top: 14px !important;
    padding-bottom: 14px !important;
    align-items: center !important;
}
main.main-content.mov-shell .po-toolbar-note {
    padding-top: 10px !important;
    padding-bottom: 10px !important;
}
main.main-content.mov-shell .po-toolbar-note span {
    line-height: 1.45 !important;
}
main.main-content.mov-shell .sticky.top-0 > .px-4 {
    padding-left: 12px !important;
...
/* ---- Voucher-like arrangement (match employee/create-voucher.php feel) ---- */
main.main-content.mov-shell {
...
/* ---- Final ERP redesign (matches reference layout) ---- */
main.main-content.mov-shell {
    background: #f8fafc !important;
...
/* Ensure supplier card does not clip dropdown panel */
.po-card.card-supplier,
.po-card.card-supplier .po-card-body,
.po-card.card-supplier .row {
    ...
    <div class="max-w-full mx-auto px-0">
        <div class="bg-white border-b border-gray-200 po-page-head">
            <div class="po-toolbar-main px-4 py-3 flex flex-wrap items-center gap-3 border-b border-gray-100">
                ...
                <div class="flex items-center gap-2 min-w-0">
                    <h1 class="text-xl font-bold text-gray-900 truncate m-0 inline-flex items-center gap-2">
                        <span>New outdoor purchase</span>
                    </h1>
                </div>
                <div class="flex-1 min-w-[8px]"></div>
                <a href="domestic_create.php" class="text-base font-medium text-gray-600 hover:text-[#2563EB] border border-gray-200 rounded-md px-3 py-2 bg-white inline-flex items-center gap-2 no-underline">
                    <i class="fas fa-truck-loading text-sm"></i> Domestic
                </a>
                <a href="create.php?type=import" class="btn px-4 py-2 rounded-md text-base font-semibold shadow-sm inline-flex items-center gap-2 border-0 no-underline" style="background-color:#2563EB;color:#fff;">
            ...
            <div class="po-toolbar-note px-4 py-2 flex flex-wrap items-center gap-2 text-base bg-gray-50/80 border-b border-gray-100">
                <span class="text-gray-600"><i class="fas fa-info-circle text-gray-400 me-1"></i>Outdoor purchases: create a shipment for tracking, then receive stock when goods arrive.</span>
            </div>
        </div>
        <div class="px-4 pt-4">
        ...
        <form method="POST" action="" id="poForm">
            <div class="row po-voucher-layout">
                <div class="col-lg-8 po-main-column">
                    ...
                    <div class="po-card card-supplier mb-4">
                        ...
                        <div class="po-card-body">
                            <div class="row g-3">
                                <div class="col-lg-4 col-md-6 d-none">
                                    <input type="hidden" id="purchase_type" name="purchase_type" value="import">
                                </div>
                                <div class="col-12">
                                    <label class="form-label">Procurement workflow</label>
                                    <div class="workflow-panel">
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="radio" name="procurement_journey" id="pj_standard" value="<?php echo htmlspecialchars(PURCHASE_PROC_STANDARD); ?>" checked>
                                            <label class="form-check-label" for="pj_standard"><strong>Standard</strong> â€” PO is pending immediately; supplier uses the portal link as today.</label>
                                        </div>
                                        <div class="form-check mb-0">
                                            <input class="form-check-input" type="radio" name="procurement_journey" id="pj_supplier_link" value="<?php echo htmlspecialchars(PURCHASE_PROC_SUPPLIER_LINK); ?>">
                                            <label class="form-check-label" for="pj_supplier_link"><strong>Supplier link</strong> â€” Save as <em>Draft</em> with quantities (estimated prices optional). You send the secure link from View PO when ready; supplier enters prices and uploads invoice, then you approve.</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12">
                                    ...
                                    <div class="pv-picker" id="pvPickerImport">
                                        <div class="pv-topbar">
                                            <div class="pv-search-wrap">
                                                ...
                                                <button type="button" class="pv-caret" id="pvPickerToggleImport"><i class="fas fa-chevron-down"></i></button>
                                            </div>
                                            <button type="button" class="pv-filter-toggle" id="pvFiltersToggleImport"><i class="fas fa-filter"></i> Filters <i class="fas fa-chevron-up" style="font-size:11px;"></i></button>
                                        </div>
                                        <div class="pv-dropdown" id="payment_voucher_dropdown">
                                            ...
                                            <div class="pv-filters" id="pv_filter_panel">
                                                <div class="pv-filter-toolbar">
                                                    ...
                                                    <button type="button" class="pv-footer-btn" id="pvFiltersToggleInlineImport"><i class="fas fa-filter"></i> Filters <i class="fas fa-chevron-up" style="font-size:11px;"></i></button>
                                ...
                                <div class="col-lg-4 col-md-6">
                                    ...
                                    <!-- Supplier Info Block -->
                                    <div id="supplierInfo" class="supplier-info-box d-none mt-3">
                                        <!-- Details will be loaded here -->
                                    </div>
                                </div>
                                <div class="col-lg-4 col-md-6">
                                    <label class="form-label">Purchase Order Date <span class="text-danger">*</span></label>
                                    <input
                                        type="date"
                                        class="form-control"
                                        id="purchase_order_date"
                                        name="purchase_order_date"
                                        value="<?php echo htmlspecialchars($selectedPurchaseOrderDate); ?>"
                                        required
                                    >
                                </div>
                                <div class="col-lg-4 col-md-6">
                                ...
                                <div class="col-lg-4 col-md-6">
                                    ...
                                    <input
                                        ...
                                        name="exchange_rate"
                                        value="<?php echo number_format((float) $selectedRate, 4, '.', ''); ?>"
                                        required
                                    >
                                ...
                                <div class="col-lg-4 col-md-6">
                                    ...
                                    <input
                                        ...
                                        value="<?php echo htmlspecialchars($selectedExpectedArrivalDate); ?>"
                                    >
                                </div>
                                <div class="col-lg-4 col-md-6">
                                    <label for="shipment_method" class="form-label">Shipment Method / Incoterms <span class="text-danger">*</span></label>
                                    <select class="form-select" id="shipment_method" name="shipment_method">
                                        <option value="">-- Select --</option>
                                        <?php foreach ($shipmentMethodOptions as $method): ?>
                                            <option value="<?php echo htmlspecialchars($method); ?>" <?php echo $selectedShipmentMethod === $method ? 'selected' : ''; ?>>
                                                <?php echo htmlspecialchars($method); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                    <small class="text-muted d-block mt-1">e.g. FOB, CIF, DDP, Air Freight, Sea Freight</small>
                                </div>
                                <div class="col-12" id="domesticFormWrap">
                                    <div class="border rounded-3 p-3 bg-light">
                                        <div class="fw-bold mb-2">Domestic Purchase Details</div>
                                        <div class="row g-3">
                                            <div class="col-md-6">
                                                ...
                                                <small class="text-muted d-block mt-1">Used for domestic receiving reference.</small>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label">Receiving</label>
                                                <div class="form-control bg-white" style="min-height: 38px;">
                                                    <span class="text-muted">You will receive this PO directly (no shipment).</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <!-- Order Items Card -->
                    <div class="po-card card-items mb-4">
                        <div class="po-card-header">
                    ...
                    <div class="po-card card-info mb-4">
                        <div class="po-card-header">
                        ...
                        <div class="po-card-body">
                            ...
                            <div class="col-md-6">
                                ...
                                <textarea class="form-control" id="notes" name="notes" rows="3" placeholder="Add any internal notes or instructions for this purchase order..."><?php echo htmlspecialchars($cloned_po['notes'] ?? ''); ?></textarea>
                            ...
                            <div class="col-md-6">
                                ...
                                <textarea class="form-control" id="terms_conditions" name="terms_conditions" rows="3" placeholder="Enter custom terms or leave blank to use company defaults..."><?php echo htmlspecialchars($cloned_po['terms_conditions'] ?? ''); ?></textarea>
                ...
                <div class="col-lg-4 po-summary-column">
                    ...
                    <div class="po-totals-card">
                        ...
                        <div class="po-totals-body">
                            ...
                            <div class="totals-row">
                                <span class="fw-medium">Shipping Estimate:</span>
                                <span class="totals-value"><?php echo $currency; ?>0.00</span>
                            </div>
...
const EXCHANGE_RATE_INPUT = document.getElementById('exchange_rate_display');
...
function syncSupplierFromVoucher() {
    ...
    const supplierByName = meta.supplierNames.find(name => {
...
function initPaymentVoucherPicker() {
    ...
    const filtersToggle = document.getElementById('pvFiltersToggleImport');
    const filtersToggleInline = document.getElementById('pvFiltersToggleInlineImport');
    ...
    const toggle = document.getElementById('pvPickerToggleImport');
    ...
    const applyDropdownPlacement = () => {
        ...
        const targetMax = Math.max(420, Math.min(900, Math.floor(available)));
        dropdown.style.maxHeight = `${targetMax}px`;

        const headerApprox = 260; // header + filters + footer spacing
        const resultsMax = Math.max(260, targetMax - headerApprox);
    ...
    document.addEventListener('click', (e) => {
        if (!e.target.closest('#pvPickerImport')) close();
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (CURRENCY_CODE_SELECT) {
        const applyCurrency = () => {
            ...
            if (EXCHANGE_RATE_INPUT) {
                EXCHANGE_RATE_INPUT.value = EXCHANGE_RATE.toFixed(4);
            }
    ...
    if (EXCHANGE_RATE_INPUT) {
        EXCHANGE_RATE_INPUT.addEventListener('input', () => {
            const manual = parseFloat(EXCHANGE_RATE_INPUT.value);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btnDom) btnDom.addEventListener('click', () => setType('domestic'));
    if (btnOut) btnOut.addEventListener('click', () => setType('import')); // Outdoor maps to import (shipment)
});
