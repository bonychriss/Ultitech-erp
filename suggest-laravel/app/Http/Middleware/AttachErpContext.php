...
/**
 * Inject ERP session context (set by public_html/suggest.php bridge).
 */
class AttachErpContext
{
    public function handle(Request $request, Closure $next): Response
    {
        $erp = $GLOBALS['ERP_SUGGEST_CONTEXT'] ?? null;
        if (!is_array($erp) || empty($erp['user_id'])) {
            abort(401, 'ERP login required. Open this page from the ERP Suggest menu.');
        }
