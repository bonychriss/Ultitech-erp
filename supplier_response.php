...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($po['status'], ['Supplier Responded', 'Approved', 'Rejected'])) {
...
<head>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            .table-po td img { width: 50px !important; height: 50px !important; }
...
<body>
    ...
    <div class="container py-5">
        ...
        <?php if(!empty($success_msg)): ?>
            <div class="alert alert-success text-center mb-4 shadow rounded-3">
        ...
        <?php else: ?>
            <div class="alert alert-success border-0 shadow-sm rounded-3 mb-4 text-center">
                 <h4 class="alert-heading fw-bold"><i class="fas fa-check-circle me-2"></i> Response Submitted</h4>
                 <p class="mb-0">You have successfully submitted your quote. The buyer is currently reviewing it.</p>
            </div>
        <?php endif; ?>
                     ...
                     <table class="table table-bordered table-po mb-0">
                        ...
                        <tbody>
                            <?php foreach($items as $index => $item): 
                                ...
                                // Image Handling
                                $imgUrl = '../../assets/images/no-image.png'; // Updated Fallback path relative from modules/purchases/ to assets is actually ../../../assets so 3 levels up? No, file is in modules/purchases/ so ../../../assets
                                // Actually supplier_response.php is in stock/modules/purchases/
                                // Root is stock/
                                // Assets are in staff/assets? or stock/assets?
                                // based on view_po.php: ../../../assets/images/Untitled.jpg worked.
                                // so ../../../assets/images/no-image.png should work.
                                $fallbackImg = '../../../assets/images/no-image.png';
                                $imgUrl = $fallbackImg;
                                
                                if (!empty($item['main_image'])) {
                                    $checkPath = "../../uploads/products/" . $item['prod_id'] . "/medium/" . $item['main_image'];
                                    // We need to check existence relative to filesystem
                                    ...
                                    <?php else: ?>
                                        <div class="input-group input-group-sm">
                                            ...
                                            <input type="number" step="0.01" min="0.01" name="items[<?php echo $item['item_id']; ?>][price]" 
                                                   ...
                                                   value="<?php echo $currentPrice > 0 ? $currentPrice : ''; ?>" 
                ...
                <?php if (!in_array($po['status'], ['Supplier Responded', 'Approved'])): ?>
                    <!-- UPLOAD INVOICE -->
                     <div class="row align-items-center bg-light p-4 rounded mb-2 border mx-0 mt-4">
                        <div class="col-md-7">
                            ...
                            <div class="form-text mt-0" data-translate="invoice_help">Required for approval. Please ensure it matches the prices above.</div>
                        </div>
                        <div class="col-md-5">
                    ...
                    <div class="text-center mt-5">
                         <button type="submit" class="btn btn-success btn-lg px-5 rounded-0 shadow hover-lift fw-bold text-uppercase ls-1">
                            <i class="fas fa-paper-plane me-2"></i> <span data-translate="submit_quote">Submit Response</span>
                         </button>
                    </div>
                <?php endif; ?>
